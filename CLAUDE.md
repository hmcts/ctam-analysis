...
- **`_bmad-output/planning-artifacts/` — the canonical, git-tracked source of truth.** Despite living under `_bmad-output/`, this subtree is committed and authoritative; do NOT treat it as scratch.
  ...
  - `epics/` — `framework.md` + `phase-1/` epics with **stories embedded inside each epic** (only Phase 1 is decomposed so far; run `bmad-create-epics-and-stories` per phase for 2–9) + `phase-0/` (dev/integration prerequisites, not part of the product's own phase sequence — see `framework.md`).
...
- **Execution units** (the 16 service/UI/infra repos) — where code lands; each gets a self-contained story packet.
