...
- **Cross-cutting changes: sweep first, don't blind find-replace.** Run a repo-wide sweep for every affected reference, use `bmad-correct-course`, clarify ambiguous terms, record the change in a Sprint Change Proposal + a `changelog.md` entry, then regenerate `docs/`. Leave dated reports and existing changelog entries as immutable history — add, don't rewrite.
- **One Sprint Change Proposal file per day** *(2026-08-26)*: `sprint-change-proposal-{date}.md`, not one file per `bmad-correct-course` run. If today's file already exists, **append** a new `## Sprint Change Proposal — {date}[suffix]` entry to it (with its own Index line at the top of that file) rather than creating a same-day lettered file (`-{date}b.md`, `-{date}c.md`, …) — that lettering habit is what previously produced 29 files needing two separate consolidation passes to undo (see `sprint-change-proposal-2026-08-26.md`). Only start a new file when the date rolls over.
