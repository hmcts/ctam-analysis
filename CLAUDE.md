...
- **`_bmad-output/planning-artifacts/` — the canonical, git-tracked source of truth.** Despite living under `_bmad-output/`, this subtree is committed and authoritative; do NOT treat it as scratch.
  ...
  - `epics/` — `framework.md` + flat `epic-<n>-<slug>.md` files with **stories embedded inside each epic**. Epic numbering is **global and monotonic** (`Epic 0`…`Epic 6`), story ids are two-part (`1.4`), and **phase is frontmatter (`phase:`), not path** — this is BMad's stock shape *(flattened 2026-08-28, SCP 2026-08-28)*. Only Phase 0 is decomposed so far; run `bmad-create-epics-and-stories` per phase for 1–8, continuing epic numbering from 7.
