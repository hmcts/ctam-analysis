...
resource: 'epics/epic-2-user-authenticates.html'
...
depends_on: [epic-0, epic-1]            # needs jo_people (1.3) + the estate
---

# Epic 2: User authenticates and lands on a role-scoped Home page
...
**Depends on Epic 1:** `jo_people` (the JOH identity-lookup target) is populated by the eLinks sync (Story 1.3); the shared Azure estate (AKS, PostgreSQL, ACR, APIM, App Insights) is provisioned by `ctam-reference-data` (Story 1.1) and consumed here.

**Vertical slice:**
- `ctam-authorisation` scaffolded from the HMCTS Crime SpringBoot template via `ctam-scaffold.sh`, following the pattern established by the first-scaffolded service (Epic 1, Story 1.1); **consumes** the shared Azure estate
...
**Out of scope (explicitly):** All upstream ingestion + `ctam-reference-data` scaffold + shared-estate provisioning + tier-(a) tables (Epic 1). Tier-(b) CTAM-owned reference data + the read API (Epic 3). FR5 machine-to-machine consumer auth (post-MVP per PRD v2.5). Real HMCTS IdP integration (mock-only at Phase 0; cuts over pre-Phase-9 per AR34). Production identity bootstrap + verification job (Epic 4).

---

## Story 2.1: Scaffold `ctam-authorisation` service from the HMCTS Crime SpringBoot template

As a **platform engineer**,
I want to scaffold `ctam-authorisation` from the HMCTS Crime SpringBoot template using `ctam-scaffold.sh`, consuming the shared Azure estate already provisioned in Epic 1,
...
**Given** the engineer has performed the GitHub manual-setup checklist (`ctam-architecture/runbooks/github-setup.md`) **before** running the scaffold (the canonical manual-setup pattern is established in Epic 1, Story 1.1):
...
**And** Group ID is `uk.gov.hmcts.ctam`, artefact is `ctam-authorisation`, base package is `uk.gov.hmcts.ctam.authorisation`, default port is 8082 (per AR3),
...
**And** a `terraform/` directory skeleton exists with per-environment stacks (`dev` / `staging` / `production`) for **this service's own resources only** (e.g. its Key Vault namespace, APIM per-API policy) — the shared estate lives in `ctam-reference-data` per AR53,
...
**Given** the shared Azure estate (AKS, encrypted-at-rest PostgreSQL Flexible Server, ACR, APIM, Application Insights / Log Analytics, Key Vault) was provisioned and independently verified in `ctam-shared-infrastructure` in **Epic 0** (AR53 revised),
...
**Given** the `ctam-architecture` Liquibase baseline changelog already created the shared `ctam_configuration_values` table ahead of `ctam-reference-data` (Epic 1, Story 1.1),
...
**And** `ctam-authorisation`'s own service-owned Liquibase changelog directory (`src/main/resources/db/changelog/`, master `db.changelog-master.yaml`) exists but is empty (auth tables created in Story 2.3).

**Given** the deployed service is publicly reachable through the shared APIM (provisioned in Epic 1),
...
## Story 2.2: User can authenticate against `ctam-mock-auth` and receive a JWT
...
**Given** the engineer has manually pre-created the private GitHub repo `ctam-mock-auth` with branch protection on `main` via the GitHub web UI (per `ctam-architecture/runbooks/github-setup.md`; the `gh` CLI is **not** available — see Story 1.1 for the canonical manual-setup pattern),
**And** runs `ctam-scaffold.sh ctam-mock-auth` (following the Story 1.1 pattern),
**When** the scaffold completes,
**Then** the service has the same baseline as Story 1.1 (Spring Boot 4.0.x, Helm chart, GitHub Actions, Actuator),
...
## Story 2.3: `ctam-authorisation` validates JWTs and resolves identity, roles, jurisdiction + Region/Area scope (read-only API)
...
**Given** `ctam-authorisation` is scaffolded per Story 2.1,
...
**And** the `ctam_authorisation` DB role owns the tables (per AR19) and holds SELECT on `jo_people` (identity lookup — `jo_people` is owned by `ctam-reference-data` and populated by the Epic 1 eLinks sync),
...
- The auth tables are created here; they're populated by Epic 4's seed scripts (dev/CI) and the production bootstrap mechanism outside the PRD's scope[^d9], not by API writes

---

## Story 2.4: Scaffold `ctam-ui` repo with React + TypeScript + Vite + GOV.UK base + auth wrapper
...
**And** the Azure Static Web App resource is provisioned via **Terraform in this repo's `terraform/` directory** (per AR53: `ctam-ui` is the first repo to need it),
...
**References:** FR55 (foundation only — Home content populated in Story 2.5), FR56; NFR17, NFR18, NFR19, NFR31, NFR40; AR42–AR45b, AR53.

---

## Story 2.5: User signs into CTAM Pathfinder via SSO and lands on a role-scoped Home page
...
**Then** they can show a stakeholder: the platform scaffold + shared estate (Epic 1, Story 1.1), JOH reference data flowing in from eLinks (Story 1.3) and MRD (Story 1.4), the auth service scaffold (Story 2.1), SSO via mock-auth (Story 2.2), two-population authorisation enforcement (Story 2.3), UI foundation (Story 2.4), and the end-to-end sign-in flow (this story),
