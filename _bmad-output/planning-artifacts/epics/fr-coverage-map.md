...
## Phase 0 (concrete epics 1–5)

| FR | Phase 0 epic / coverage | Post-MVP residual? | Notes |
|---|---|---|---|
| FR1 | [Epic 2](epic-2-user-authenticates.md) (sign-in + two-population identity resolution, Stories 2.3/2.5) + [Epic 1](epic-1-upstream-reference-data-ingested.md) (`jo_people` ingested — the JOH lookup target) + [Epic 4](epic-4-user-populations-bootstrapped.md) (the auth records resolved against) | — | IdP email → `jo_people` → `personnel_number` → `ctam_joh_identities` UUID (JOH) or `ctam_auth_staff_identities` UUID (staff),[^d9] |
| FR2 | [Epic 2](epic-2-user-authenticates.md) Story 2.3 | — | Authorisation principal → roles + **jurisdiction** + Region/Area scope (read-only API) |
| FR3 | [Epic 2](epic-2-user-authenticates.md) Story 2.3 | — | `GET /v1/users/{id}/effective-permissions` |
| **FR4** | **Data layer only** in [Epic 4](epic-4-user-populations-bootstrapped.md) (auth tables maintainable by DBAs via SQL per the identity-bootstrap runbook) | **YES — UI surface** for sysadmins **is post-MVP**[^d10] | Bootstrap mechanism itself is outside the PRD's scope[^d9] |
| FR5 | — | — | Post-MVP per PRD v2.5 (intentional deferral; pre-existing) |
| **FR6** | **Tier (a)**: [Epic 1](epic-1-upstream-reference-data-ingested.md) Story 1.2 (tables) + Stories 1.3/1.4 (eLinks sync + MRD ingestion — read-only in CTAM, corrections at source). **Tier (b)**: [Epic 3](epic-3-reference-data-read-only-api.md) Story 3.1 (tables + seed + DBA runbook). Read API over both tiers: Story 3.2. | **YES — tier-(b) RSU maintenance UI** is post-MVP[^d10]. Tier (a) never gets a CTAM write surface in any phase. | Two ownership tiers in separate tables preserve lineage |
| FR7 | [Epic 1](epic-1-upstream-reference-data-ingested.md) Story 1.2 (tier-(a) grants) + [Epic 3](epic-3-reference-data-read-only-api.md) Story 3.1 (tier-(b) grants + pattern complete) | — | Direct SQL via SELECT grants; **writes follow the tier** (ingestion for tier (a); DBA SQL for tier (b)) |
| FR8 | distributed (lands in [Epic 1](epic-1-upstream-reference-data-ingested.md) Story 1.1 first) | — | Shared `ctam_configuration_values` Liquibase baseline changelog |
| FR9 | [Epic 5](epic-5-system-dispatches-emails.md) | — | Notification dispatch + delivery log (`ctam_notification_dispatches`). User-JWT propagation only at Phase 0; `client_credentials` flow moved to Phase 6 |
| FR55 | [Epic 2](epic-2-user-authenticates.md) Story 2.5 | — | Home shell with role-scoped navigation |
| FR56 | [Epic 2](epic-2-user-authenticates.md) (business stack) | **Partial — admin stack** is post-MVP[^d10] | `ctam-ui` delivered in Phase 0; `ctam-admin-ui` post-MVP |
| **FR57** | [Epic 4](epic-4-user-populations-bootstrapped.md) (initial all-FALSE flag state keyed by (jurisdiction, region)) — cutover orchestration in Phase 9+ (per-wave flip via direct SQL by DBA) | **Partial — activation toggle UI** is post-MVP | Cutover: `UPDATE ctam_auth_user_activation_flags SET activated = TRUE WHERE jurisdiction = '…' AND region = '…'` per rollout runbook |
| **FR58** | [Epic 2](epic-2-user-authenticates.md) Story 2.3 (Authorisation read API — first exercise) + [Epic 3](epic-3-reference-data-read-only-api.md) Story 3.2 (Reference Data read API) + every service story | — | API-as-Product read-side standards |
| **FR59** | [Epic 1](epic-1-upstream-reference-data-ingested.md) Story 1.1 (first exercise — first scaffolded service) + every service story | — | Structured logs + correlation IDs |
| *(NFR24)* | [Epic 1](epic-1-upstream-reference-data-ingested.md) Stories 1.3/1.4 | MRD reader swaps to API post-MVP (when MRD APIs ship) | JOH eLinks + MRD are MVP integrations[^d11] — the programme's first deliverable |

> There is no data-migration FR[^d3]. Reference data arrives via upstream ingestion (Epic 1); user records are bootstrapped outside the PRD's scope[^d9]; tier-(a) corrections happen at source and tier-(b) changes follow the DBA runbook change-trail.
