...
## Phase 0 (concrete epics 0.0–0.9)

| FR | Phase 0 epic / coverage | Post-MVP residual? | Notes |
|---|---|---|---|
| FR1 | [Epic 0.4](phase-0/epic-0.4-user-authenticates.md) (sign-in + two-population identity resolution, Stories 0.4.3/0.4.5) + [Epic 0.1](phase-0/epic-0.1-postgres-db-schema-design.md) (`jo_people` schema) + [Epic 0.2](phase-0/epic-0.2-joh-reference-data-etl-process.md) (`jo_people` populated via the eLinks sync — the JOH lookup target) + [Epic 0.6](phase-0/epic-0.6-user-populations-bootstrapped.md) (the auth records resolved against) | — | IdP email → `jo_people` → `personnel_number` → `ctam_joh_identities` UUID (JOH) or `ctam_auth_staff_identities` UUID (staff),[^d9] |
...
| **FR4** | **Data layer only** in [Epic 0.6](phase-0/epic-0.6-user-populations-bootstrapped.md) (auth tables maintainable by DBAs via SQL per the identity-bootstrap runbook) | **YES — UI surface** for sysadmins **is post-MVP**[^d10] | Bootstrap mechanism itself is outside the PRD's scope[^d9] |
| FR5 | — | — | Post-MVP per PRD v2.5 (intentional deferral; pre-existing) |
| **FR6** | **Tier (a)**: [Epic 0.1](phase-0/epic-0.1-postgres-db-schema-design.md) Story 0.1.2 (tables) + [Epic 0.2](phase-0/epic-0.2-joh-reference-data-etl-process.md) Story 0.2.1 (eLinks sync) + [Epic 0.3](phase-0/epic-0.3-mrd-reference-data-etl-process.md) Story 0.3.1 (MRD ingestion) — read-only in CTAM, corrections at source. **Tier (b)**: [Epic 0.5](phase-0/epic-0.5-joh-reference-data-read-api.md) Story 0.5.1 (tables + seed + DBA runbook). Read API: tier (b) + JOH tier (a), Story 0.5.2; MRD tier (a) via [Epic 0.9](phase-0/epic-0.9-mrd-reference-data-read-api.md) Story 0.9.1. | **YES — tier-(b) RSU maintenance UI** is post-MVP[^d10]. Tier (a) never gets a CTAM write surface in any phase. | Two ownership tiers in separate tables preserve lineage |
| FR7 | [Epic 0.1](phase-0/epic-0.1-postgres-db-schema-design.md) Story 0.1.2 (tier-(a) grants) + [Epic 0.5](phase-0/epic-0.5-joh-reference-data-read-api.md) Story 0.5.1 (tier-(b) grants + pattern complete) + [Epic 0.9](phase-0/epic-0.9-mrd-reference-data-read-api.md) Story 0.9.1 (MRD read surface) | — | Direct SQL via SELECT grants; **writes follow the tier** (ingestion for tier (a); DBA SQL for tier (b)) |
| FR8 | distributed (lands in [Epic 0.1](phase-0/epic-0.1-postgres-db-schema-design.md) Story 0.1.1 first) | — | Shared `ctam_configuration_values` Liquibase baseline changelog |
| FR9 | [Epic 0.7](phase-0/epic-0.7-system-dispatches-emails.md) | — | Notification dispatch + delivery log (`ctam_notification_dispatches`). User-JWT propagation only at Phase 0; `client_credentials` flow moved to Phase 6 |
...
| **FR57** | [Epic 0.6](phase-0/epic-0.6-user-populations-bootstrapped.md) (initial all-FALSE flag state keyed by (jurisdiction, region)) — cutover orchestration in Phase 9+ (per-wave flip via direct SQL by DBA) | **Partial — activation toggle UI** is post-MVP | Cutover: `UPDATE ctam_auth_user_activation_flags SET activated = TRUE WHERE jurisdiction = '…' AND region = '…'` per rollout runbook |
| **FR58** | [Epic 0.4](phase-0/epic-0.4-user-authenticates.md) Story 0.4.3 (Authorisation read API — first exercise) + [Epic 0.5](phase-0/epic-0.5-joh-reference-data-read-api.md) Story 0.5.2 (Reference Data read API) + [Epic 0.9](phase-0/epic-0.9-mrd-reference-data-read-api.md) Story 0.9.1 (extends the same spec) + every service story | — | API-as-Product read-side standards |
...
| *(NFR24)* | [Epic 0.2](phase-0/epic-0.2-joh-reference-data-etl-process.md) Story 0.2.1 + [Epic 0.3](phase-0/epic-0.3-mrd-reference-data-etl-process.md) Story 0.3.1 | MRD reader swaps to API post-MVP (when MRD APIs ship) | JOH eLinks + MRD are MVP integrations[^d11] — the programme's first deliverable |
