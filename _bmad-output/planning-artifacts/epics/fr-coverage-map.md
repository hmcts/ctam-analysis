...
|---|---|---|---|
| FR1 | [Epic 0.4](phase-0/epic-0.4-user-authenticates.md) (sign-in + two-population identity resolution, Stories 0.4.3/0.4.5) + [Epic 0.1](phase-0/epic-0.1-postgres-db-schema-design.md) (`jo_people` schema) + [Epic 0.2](phase-0/epic-0.2-joh-reference-data-etl-process.md) (`jo_people` populated via the eLinks sync — the JOH lookup target) + [Epic 0.7](phase-0/epic-0.7-user-populations-bootstrapped.md) (the auth records resolved against) | — | IdP email → `jo_people` → `personnel_number` → `ctam_joh_identities` UUID (JOH) or `ctam_auth_staff_identities` UUID (staff),[^d9] |
...
| **FR6** | **Tier (a)**: [Epic 0.1](phase-0/epic-0.1-postgres-db-schema-design.md) Story 0.1.2 (tables) + [Epic 0.2](phase-0/epic-0.2-joh-reference-data-etl-process.md) Story 0.2.1 (eLinks sync) + [Epic 0.3](phase-0/epic-0.3-mrd-reference-data-etl-process.md) Story 0.3.1 (MRD ingestion) — read-only in CTAM, corrections at source. **Tier (b)**: [Epic 0.5](phase-0/epic-0.5-joh-reference-data-read-api.md) Story 0.5.1 (tables + seed + DBA runbook). Read API: tier (b) + JOH tier (a), Story 0.5.2; MRD tier (a) via [Epic 0.6](phase-0/epic-0.6-mrd-reference-data-read-api.md) Story 0.6.1. | **YES — tier-(b) RSU maintenance UI** is post-MVP[^d10]. Tier (a) never gets a CTAM write surface in any phase. | Two ownership tiers in separate tables preserve lineage |
| FR7 | [Epic 0.1](phase-0/epic-0.1-postgres-db-schema-design.md) Story 0.1.2 (tier-(a) grants) + [Epic 0.5](phase-0/epic-0.5-joh-reference-data-read-api.md) Story 0.5.1 (tier-(b) grants + pattern complete) + [Epic 0.6](phase-0/epic-0.6-mrd-reference-data-read-api.md) Story 0.6.1 (MRD read surface) | — | Direct SQL via SELECT grants; **writes follow the tier** (ingestion for tier (a); DBA SQL for tier (b)) |
