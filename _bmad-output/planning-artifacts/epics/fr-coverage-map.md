...
| **FR6** | **Tier (a)**: [Epic 0.1](phase-0/epic-0.1-upstream-reference-data-ingested.md) Story 0.1.2 (tables) + Stories 0.1.3/0.1.4 (eLinks sync + MRD ingestion — read-only in CTAM, corrections at source). **Tier (b)**: [Epic 0.3](phase-0/epic-0.3-reference-data-read-only-api.md) Story 0.3.1 (tables + seed + DBA runbook). Read API over both tiers: Story 0.3.2. | **YES — tier-(b) RSU maintenance UI** is post-MVP[^d10]. Tier (a) never gets a CTAM write surface in any phase. | Two ownership tiers in separate tables preserve lineage |
...
| FR8 | distributed (lands in [Epic 0.1](phase-0/epic-0.1-upstream-reference-data-ingested.md) Story 0.1.1 first) | — | Shared `ctam_configuration_values` Liquibase baseline changelog |
...
| **FR59** | [Epic 0.1](phase-0/epic-0.1-upstream-reference-data-ingested.md) Story 0.1.1 (first exercise — first scaffolded service) + every service story | — | Structured logs + correlation IDs |
| *(NFR24)* | [Epic 0.1](phase-0/epic-0.1-upstream-reference-data-ingested.md) Stories 0.1.3/0.1.4 | MRD reader swaps to API post-MVP (when MRD APIs ship) | JOH eLinks + MRD are MVP integrations[^d11] — the programme's first deliverable |

> There is no data-migration FR[^d3]. Reference data arrives via upstream ingestion (Epic 0.1); user records are bootstrapped outside the PRD's scope[^d9]; tier-(a) corrections happen at source and tier-(b) changes follow the DBA runbook change-trail.
