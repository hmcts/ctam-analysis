...
## Phase 0 (concrete epics 0.1–0.5)

| FR | Phase 0 epic / coverage | Post-MVP residual? | Notes |
|---|---|---|---|
| FR1 | [Epic 0.2](phase-0/epic-0.2-user-authenticates.md) (sign-in + two-population identity resolution, Stories 0.2.3/0.2.5) + [Epic 0.1](phase-0/epic-0.1-ctam-postgres-db-schema-design.md) (`jo_people` schema) + [Epic 0.7](phase-0/epic-0.7-joh-reference-data-etl-process.md) (`jo_people` populated via the eLinks sync — the JOH lookup target) + [Epic 0.4](phase-0/epic-0.4-user-populations-bootstrapped.md) (the auth records resolved against) | — | IdP email → `jo_people` → `personnel_number` → `ctam_joh_identities` UUID (JOH) or `ctam_auth_staff_identities` UUID (staff),[^d9] |
...
| **FR6** | **Tier (a)**: [Epic 0.1](phase-0/epic-0.1-ctam-postgres-db-schema-design.md) Story 0.1.2 (tables) + [Epic 0.7](phase-0/epic-0.7-joh-reference-data-etl-process.md) Story 0.7.1 (eLinks sync) + [Epic 0.8](phase-0/epic-0.8-mrd-reference-data-etl-process.md) Story 0.8.1 (MRD ingestion) — read-only in CTAM, corrections at source. **Tier (b)**: [Epic 0.3](phase-0/epic-0.3-reference-data-read-only-api.md) Story 0.3.1 (tables + seed + DBA runbook). Read API over both tiers: Story 0.3.2. | **YES — tier-(b) RSU maintenance UI** is post-MVP[^d10]. Tier (a) never gets a CTAM write surface in any phase. | Two ownership tiers in separate tables preserve lineage |
| FR7 | [Epic 0.1](phase-0/epic-0.1-ctam-postgres-db-schema-design.md) Story 0.1.2 (tier-(a) grants) + [Epic 0.3](phase-0/epic-0.3-reference-data-read-only-api.md) Story 0.3.1 (tier-(b) grants + pattern complete) | — | Direct SQL via SELECT grants; **writes follow the tier** (ingestion for tier (a); DBA SQL for tier (b)) |
| FR8 | distributed (lands in [Epic 0.1](phase-0/epic-0.1-ctam-postgres-db-schema-design.md) Story 0.1.1 first) | — | Shared `ctam_configuration_values` Liquibase baseline changelog |
...
| FR56 | [Epic 0.2](phase-0/epic-0.2-user-authenticates.md) (business stack) | **Partial — admin stack** is post-MVP[^d10] | `ctam-ui` delivered in Phase 0; `ctam-admin-ui` post-MVP |
| **FR57** | [Epic 0.4](phase-0/epic-0.4-user-populations-bootstrapped.md) (initial all-FALSE flag state keyed by (jurisdiction, region)) — cutover orchestration in Phase 9+ (per-wave flip via direct SQL by DBA) | **Partial — activation toggle UI** is post-MVP | Cutover: `UPDATE ctam_auth_user_activation_flags SET activated = TRUE WHERE jurisdiction = '…' AND region = '…'` per rollout runbook |
| **FR58** | [Epic 0.2](phase-0/epic-0.2-user-authenticates.md) Story 0.2.3 (Authorisation read API — first exercise) + [Epic 0.3](phase-0/epic-0.3-reference-data-read-only-api.md) Story 0.3.2 (Reference Data read API) + every service story | — | API-as-Product read-side standards |
| **FR59** | [Epic 0.1](phase-0/epic-0.1-ctam-postgres-db-schema-design.md) Story 0.1.1 (first exercise — first scaffolded service) + every service story | — | Structured logs + correlation IDs |
| *(NFR24)* | [Epic 0.7](phase-0/epic-0.7-joh-reference-data-etl-process.md) Story 0.7.1 + [Epic 0.8](phase-0/epic-0.8-mrd-reference-data-etl-process.md) Story 0.8.1 | MRD reader swaps to API post-MVP (when MRD APIs ship) | JOH eLinks + MRD are MVP integrations[^d11] — the programme's first deliverable |

> There is no data-migration FR[^d3]. Reference data arrives via upstream ingestion (Epic 0.1 schema + Epics 0.7/0.8 ETL processes); user records are bootstrapped outside the PRD's scope[^d9]; tier-(a) corrections happen at source and tier-(b) changes follow the DBA runbook change-trail.
