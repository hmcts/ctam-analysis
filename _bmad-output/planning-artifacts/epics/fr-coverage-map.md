...
## Phase 0 (concrete epics 0.0–0.8)
...
| **FR6** | **Tier (a)**: [Epic 0.1](phase-0/epic-0.1-postgres-db-schema-design.md) Story 0.1.2 (tables) + [Epic 0.2](phase-0/epic-0.2-joh-reference-data-etl-process.md) Story 0.2.1 (eLinks sync) + [Epic 0.3](phase-0/epic-0.3-mrd-reference-data-etl-process.md) Story 0.3.1 (MRD ingestion) — read-only in CTAM, corrections at source. **Tier (b)**: [Epic 0.5](phase-0/epic-0.5-reference-data-read-only-api.md) Story 0.5.1 (tables + seed + DBA runbook). Read API over both tiers: Story 0.5.2. | **YES — tier-(b) RSU maintenance UI** is post-MVP[^d10]. Tier (a) never gets a CTAM write surface in any phase. | Two ownership tiers in separate tables preserve lineage |
| FR7 | [Epic 0.1](phase-0/epic-0.1-postgres-db-schema-design.md) Story 0.1.2 (tier-(a) grants) + [Epic 0.5](phase-0/epic-0.5-reference-data-read-only-api.md) Story 0.5.1 (tier-(b) grants + pattern complete) | — | Direct SQL via SELECT grants; **writes follow the tier** (ingestion for tier (a); DBA SQL for tier (b)) |
...
| **FR58** | [Epic 0.4](phase-0/epic-0.4-user-authenticates.md) Story 0.4.3 (Authorisation read API — first exercise) + [Epic 0.5](phase-0/epic-0.5-reference-data-read-only-api.md) Story 0.5.2 (Reference Data read API) + every service story | — | API-as-Product read-side standards |
