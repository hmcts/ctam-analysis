...
description: 'User outcome: Judicial-holder reference data flows into CTAM from its upstream sources of truth — the JOH eLinks API (15 jo_ entities, nightly in-process sync, Story 1.3) and the MRD weekly Excel…'
resource: 'epics/phase-0-overview.html'
...
> Phase 0 is sequenced **platform-then-integrations-first**: Epic 0 platform estate → Epic 1 ingestion → Epic 2 auth + UI → Epic 3 read API → Epic 4 bootstrap → Epic 5 notification. The shared Azure estate stands up and is independently verified first (Epic 0, `ctam-shared-infrastructure`); the ingestion then runs in-process inside `ctam-reference-data` — there is no `ctam-integrations` repo.
...
>
> The Phase 0 areas in [framework.md](framework.md) are an **architectural map**. The seven concrete user-value epics below are the **implementation plan** — each delivers a demoable user outcome and consolidates the supporting technical work as stories within the epic.
...
- **Platform estate is Epic 0 — the first deliverable**: the shared Azure estate (AKS, PostgreSQL, ACR, APIM, App Insights, Key Vault) is provisioned via Terraform in the dedicated `ctam-shared-infrastructure` repo and independently verified layer-by-layer (AR53 revised, HMCTS CNP `{product}-shared-infrastructure` standard).
- **Upstream ingestion is Epic 1 — the first domain deliverable**: eLinks sync (Story 1.3) + MRD ingestion (Story 1.4). `ctam-reference-data` is the first domain service scaffolded and deploys onto the Epic 0 estate. In-process — **no `ctam-integrations` repo**.
...
|---|---|---|---|
| [0](epic-0-platform-estate-provisioned.md) | Platform estate is provisioned, verifiable, and CNP-compliant | 5 | 🟡 Planned |
| [1](epic-1-upstream-reference-data-ingested.md) | Upstream JOH/MRD reference data is ingested | 4 | 🟡 Planned |
| [2](epic-2-user-authenticates.md) | User authenticates and lands on a role-scoped Home page | 5 | 🟡 Planned |
| [3](epic-3-reference-data-read-only-api.md) | Reference data is served read-only via a versioned, jurisdiction-filtered API | 2 | 🟡 Planned |
| [4](epic-4-user-populations-bootstrapped.md) | Both user populations are bootstrapped and verifiable against the IdP | 1 | 🟡 Planned |
| [5](epic-5-system-dispatches-emails.md) | Notification service is scaffolded and contractually ready | 2 | 🟡 Planned |
| [6](epic-6-context-bus-and-shared-baseline.md) | Context bus is published and the shared configuration baseline exists | 2 | 🟢 In progress |
| **Total** | | **21 stories** | |

## Epic summaries

#### Epic 0: Platform estate is provisioned, verifiable, and CNP-compliant (5 stories)

**User outcome:** The shared Azure estate — AKS, PostgreSQL Flexible Server, ACR, APIM, Application Insights, Key Vault — is stood up via **Terraform** in its own dedicated repo, **`ctam-shared-infrastructure`** (HMCTS CNP `{product}-shared-infrastructure` standard, AR53 revised), provisioned **layer-by-layer with each layer independently verified at deploy time**, so every Phase 0 service has a tested platform to deploy onto. Precedes Epic 1. Stories: 0.1 repo + Terraform foundation (state backend, plan/apply CI); 0.2 network + AKS (verified via `kubectl get nodes` + hello pod); 0.3 PostgreSQL + Key Vault (TLS-only connect, plaintext refused, secret round-trip); 0.4 ACR + observability (image pull, test trace lands); 0.5 APIM + smoke API (gateway → echo 200 over TLS).

**FRs covered:** none (foundational platform infrastructure). **NFRs:** NFR10, NFR11, NFR16, NFR25–NFR28, NFR31. **ARs:** AR53 (revised), A34, G9.

→ [Full epic with stories](epic-0-platform-estate-provisioned.md)

#### Epic 1: Upstream JOH/MRD reference data is ingested (4 stories)

**User outcome:** Judicial-holder reference data flows into CTAM from its upstream sources of truth — the **JOH eLinks API** (15 `jo_*` entities, nightly in-process sync, Story 1.3) and the **MRD** weekly Excel feed (`mrd_*`, Story 1.4) — so `jo_people` exists and is current. This is the platform's foundational data layer. `ctam-reference-data` is the first domain service scaffolded (Story 1.1) and deploys onto the shared estate provisioned in **Epic 0**; tier-(a) tables + write protection are Story 1.2.
...
→ [Full epic with stories](epic-1-upstream-reference-data-ingested.md)

#### Epic 2: User authenticates and lands on a role-scoped Home page (5 stories)

**User outcome:** A user from **either identity population** — JOH (Judge, Tribunal Judge, Tribunal Member) or HMCTS admin staff (RSU, Court user, Tribunal Caseworker, Finance, MI) — signs in via SSO, has their canonical identity resolved (CTAM JOH UUID via the eLinks-synced `jo_people` → `personnel_number` → `ctam_joh_identities`; staff UUID via `ctam_auth_staff_identities`), and lands on a role-scoped Home page. Depends on Epic 1 (`jo_people` populated) and consumes the shared estate provisioned in Epic 0.

**FRs covered:** FR1, FR2, FR3, FR55, FR56 (business stack); FR57 (activation surface), FR58 (Authorisation read API)

→ [Full epic with stories](epic-2-user-authenticates.md)

#### Epic 3: Reference data is served read-only via a versioned, jurisdiction-filtered API (2 stories)

**User outcome:** Tier-(b) CTAM-owned reference data (regions, offices, calendar, operational vocabularies) is created, seeded, and DBA-maintained per runbook[^d10] (Story 3.1); all reference data — both tiers — is served by the versioned **read-only**, **jurisdiction-filtered** REST API (Story 3.2). Sequenced after Epic 2 (depends on `JWTFilter` + `authz/check`).

**FRs covered (Phase 0 surface):** FR6 (tier-(b) maintenance + read API over both tiers), FR7, FR58

→ [Full epic with stories](epic-3-reference-data-read-only-api.md)

#### Epic 4: Both user populations are bootstrapped and verifiable against the IdP (1 story)
...
→ [Full epic with stories](epic-4-user-populations-bootstrapped.md)

#### Epic 5: Notification service is scaffolded and contractually ready (2 stories)
...
→ [Full epic with stories](epic-5-system-dispatches-emails.md)

#### Epic 6: Context bus is published and the shared configuration baseline exists (2 stories)
...
**Runs between 0 and 1** — the number is not the order; see `depends_on` in its frontmatter. Promoted from the `arch-baseline` dispatch-graph node (SCP 2026-08-19d) so BMad tracks it.

**FRs covered:** FR8

→ [Full epic with stories](epic-6-context-bus-and-shared-baseline.md)

## Phase 0 Epic Stories Summary

| Epic | Stories | FRs covered | Phase 0 demo |
|---|---|---|---|
| 0 | 5 stories (0.1–0.5) | none (platform infra); NFR10, NFR11, NFR16, NFR25–NFR28, NFR31 | Each Terraform layer stands up and is verified as deployed — `kubectl get nodes` Ready across AZs, PostgreSQL TLS-only (plaintext refused), Key Vault secret round-trip, ACR image pull, APIM smoke API → 200 over TLS |
| 1 | 4 stories (1.1–1.4) | FR1 (`jo_people` target), FR6 tier (a), FR7 tier (a), FR8, FR59; NFR24 | JOH/MRD reference data flows in from eLinks + MRD → `jo_people` current (verified via `ctam_sync_status` + CI WireMock stub) |
| 2 | 5 stories (2.1–2.5) | FR1, FR2, FR3, FR55, FR56, FR57 (activation surface), FR58 | User (either population) signs in via mock-auth → `ctam-authorisation` resolves identity/roles/jurisdiction → role-scoped Home renders |
| 3 | 2 stories (3.1–3.2) | FR6 (tier b + read API), FR7, FR58 | Jurisdiction-filtered Reference Data API serves both tiers read-only |
| 4 | 1 story (4.1) | FR1 (lookup data), FR4 (data layer), FR57 (flag bootstrap) | Seeded users across both populations verified against the IdP; Epic 2 sign-in works against them |
| 6 | 2 stories (6.1–6.2) | FR8; NFR16, NFR40 | A service repo pins `arch-v1.0` and resolves `_arch/`; a service role SELECTs from `ctam_configuration_values` and is refused a write |
| 5 | 2 stories (5.1–5.2) | FR9 | `POST /v1/notifications/send` works end-to-end via Postman against Mailpit |
| **Total** | **21 stories** | | The seven demos chain together for the Phase 0 stakeholder walkthrough — starting with the verified platform estate |
