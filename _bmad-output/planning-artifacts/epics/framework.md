...
description: 'CTAM Pathfinder is built in 10 sequential phases (1–10+) per the PRD''s Phase-by-Phase Journey Mapping and the architecture''s Repository Strategy:'
...
CTAM Pathfinder is built in **10 sequential phases (1–10+)** per the PRD's Phase-by-Phase Journey Mapping and the architecture's Repository Strategy:

- **Phase 1** is cross-cutting foundations (multiple parallel areas).
- **Phases 2–9** each deliver one service end-to-end (backend + UI module).
- **Phase 10+** is the jurisdiction-first wave rollout[^d8][^d13]: wave 1 = the **Employment Tribunals (ET) jurisdiction** (incumbent `[ET-INCUMBENT-TBD]`, gap G8.4); wave 2 = the **SSCS jurisdiction** (replacing ListAssist; GAPS case management retained); waves 3+ = Courts jurisdictions per-region (replacing APEX/JI).

The first level of grouping below is **Phase** (delivery sequence); the second level is **Area** (the capability or cross-cutting concern that anchors the epic). Within each Area, concrete epics with stories and Gherkin acceptance criteria live in the per-phase folders (e.g. [phase-1/](phase-1/index.md)).

## Epic Phase × Area Summary

| Phase | Area | Component(s) | Primary FR/NFR coverage |
|---|---|---|---|
| **0** | Dev/Integration Prerequisites | `ctam-jomockapi` (local mock of the JOH eLinks API — dev/integration-only, never deployed to production, like `ctam-mock-auth`) | none directly; supports FR1, FR6, FR7 indirectly via Epic 1.1 |
| **1** | Platform & DevEx | `ctam-architecture` (scaffolding), GitHub Actions, APIM, AKS, Application Insights, shared `ctam_configuration_values` | FR8, FR58, FR59, NFR25–NFR28, NFR40, NFR42 |
| **1** | Identity & Authorisation | `ctam-mock-auth`, `ctam-authorisation` (two-population identity resolution; jurisdiction-aware) | FR1–FR4, FR57 *(flag surface)*, NFR12, NFR13 |
| **1** | Upstream Reference-Data Ingestion | `ctam-reference-data` in-process eLinks sync + MRD blob ingestion (tier (a): `jo_*`, `mrd_*`, `ctam_sync_status`) | FR6 *(tier a)*, FR7, NFR24 |
| **1** | Reference Data (tier (b) + read API) | `ctam-reference-data` (backend); maintenance UI post-MVP in `ctam-admin-ui`[^d10] | FR6, FR7 |
| **1** | Notification | `ctam-notification` | FR9, NFR22 |
| **1** | Identity Bootstrap & Verification | seed scripts + bootstrap-verification job + runbook (`ctam-architecture`) | FR1 *(data)*, FR4 *(data layer)*, FR57 *(initial flags)* |
| **1** | Business UI Foundation | `ctam-ui` (shell, auth, design system) | FR55 *(shell)*, FR56 *(stack)*, NFR17 |
| **1 → post-MVP** | Admin UI Foundation *(post-MVP[^d10])* | `ctam-admin-ui` (shell, auth, design system, tier-(b) Reference Data maintenance, User & Role admin) | FR4 *(UI surface)*, FR6 *(tier-(b) UI surface)*, FR56 *(stack)*, NFR17 |
| **2** | JOH Records & Working Patterns | `ctam-joh` + UI module | FR10–FR18 |
| **3** | Absence Workflow | `ctam-absence` + UI module | FR19–FR22 |
| **4** | Vacancy & Cover | `ctam-vacancy` + UI module | FR23–FR28 |
| **5** | Booking Management | `ctam-booking` + UI module | FR29–FR34 |
| **6** | Sitting Management | `ctam-sitting` + UI module | FR35–FR40 |
| **7** | Payment Processing & Reconciliation | `ctam-payment` + UI module | FR41 *(part)*, FR44, FR46, FR47, NFR21, NFR35 |
| **7** | Payment Batch | `ctam-payment-batch` (scheduled) | FR42, FR43, FR45 |
| **8** | Itineraries Read Model | `ctam-itinerary` *(no own tables)* + UI views | FR48–FR52, NFR8, NFR37 |
| **9** | MI Feed & Reporting | `ctam-mi-feed` *(no own tables)* + Reports UI module | FR53, FR54, NFR23 |
| **10+** | Wave Rollout (jurisdiction-first) | per-(jurisdiction, region) activation, incumbent-experienced manual UAT, rollback playbook, **ET-cohort readiness gate** | FR57 *(activation)*, FR60, NFR36, NFR38, NFR41 |

Cross-cutting NFRs (performance NFR1–NFR9, security/data NFR10–NFR16, NFR30–NFR33, accessibility NFR17–NFR19, maintainability NFR39) are inherited by every phase; their architectural support lives in Phase 1 (Platform & DevEx) and is exercised in every domain phase.

## Phase 0 — Dev/Integration Prerequisites

> Phase 0 is **not part of the product's own phase sequence** — it holds dev/integration-only prerequisites, the same category as `ctam-mock-auth`, never deployed to production. It precedes Phase 1 numerically because its one epic is a prerequisite for Phase 1's ingestion work, not because it delivers product scope. Added 2026-08-24 (SCP 2026-08-24b).

### Phase 0 · Area: Dev/Integration Prerequisites

**Scope**: A local mock of the upstream **Judiciary E-links People API (v5)** (`ctam-jomockapi`), built from the real swagger doc and example payloads, serving real production reference-data exports plus 100 generated JOH people. Gives `ctam-reference-data`'s ingestion sync (Epic 1.1) a realistic, stable contract to build and test against ahead of the real eLinks API's contract being confirmed (gaps.md G8.1).

**Component(s)**: `ctam-jomockapi` (dev/integration-only, never deployed to production).

**Concrete epic in this area**: [Epic 0.0](phase-0/epic-0.0-joh-mock-apis.md).

**Primary FR/NFR coverage**: none directly; supports FR1, FR6, FR7 indirectly via Epic 1.1.

→ **Phase 0 concrete epic:** [phase-0/](phase-0/index.md)

## Phase 1 — Foundations

> Phase 1 is the platform smoke-test (per PRD Key Characteristic 4). All API-as-Product standards (versioning, OpenAPI, [RFC 9457](https://datatracker.ietf.org/doc/html/rfc9457), `Deprecation`/`Sunset`) are exercised on Reference Data reads and Authorisation lookups before any domain service is built.
>
> The Areas below are the **architectural map**. The **implementation plan** is the four concrete user-value epics in [phase-1/](phase-1/index.md).

### Phase 1 · Area: Platform & DevEx

**Scope**: **Terraform provisioning of the Azure estate** (HMCTS standard, per AR53 revised — shared estate in the dedicated `ctam-shared-infrastructure` repo, provisioned in Epic 1.0; per-service resources in each service's `terraform/`). Service scaffolding (`ctam-scaffold.sh` over HMCTS Crime SpringBoot template, incl. the per-repo `terraform/` skeleton), per-service GitHub Actions pipeline (`ci.yml` + `deploy-{env}.yml` + per-region per-wave gated production deploy), OpenAPI/Spectral/ArchUnit/Spotless/Checkstyle tooling, structured Logback JSON logging conventions, OpenTelemetry → Application Insights wiring, shared `ctam_configuration_values` infrastructure table managed by `ctam-architecture` Liquibase baseline changelog, Azure API Management at the edge (rate limits, deprecation headers, `/actuator/*` restriction), AKS UK South multi-AZ HA, Helm chart conventions, Azure Key Vault integration.
...
**Concrete epics in this area**: [Epic 1.0](phase-1/epic-1.0-platform-estate-provisioned.md) (the Azure estate) and [Epic 1.6](phase-1/epic-1.6-context-bus-and-shared-baseline.md) (the published context bus + the shared `ctam_configuration_values` baseline).

**Primary FR/NFR coverage**: FR8, FR58, FR59, NFR25–NFR28, NFR40, NFR42; underpins every AR1–AR52.

### Phase 1 · Area: Identity & Authorisation
...
**Primary FR/NFR coverage**: FR1, FR2, FR3, FR4, FR57 *(flags wired here; wave activation orchestrated in Phase 10+)*; NFR12, NFR13, NFR16, NFR20. *(FR5 is post-MVP; out of scope here.)*

### Phase 1 · Area: Reference Data
...
**Ingestion (in Epic 1.1's vertical slice — sign-in depends on `jo_people`)**: nightly in-process `@Scheduled` eLinks sync (full-refresh upsert on upstream natural keys; soft-deactivation, never hard-delete; run log in `ctam_sync_status`) + weekly MRD Excel via Azure Blob drop (validate / upsert / archive; idempotent per file; reader swaps for the MRD API post-MVP). Per AR46–AR49.
...
### Phase 1 · Area: Business UI Foundation
...
Phase 1's dependencies are structured data, held in each epic's frontmatter (`repo:` and `depends_on:`) — that is what `scripts/dispatch-preflight.sh` reads. Phases 2–9 are **not yet decomposed into epics**, so their edges are recorded here as prose until `bmad-create-epics-and-stories` runs for each phase and gives them frontmatter of their own.

| Phase | Repo(s) | Depends on | Why |
|---|---|---|---|
| **0 — Dev/Integration Prerequisites** | `ctam-jomockapi` | none | standalone local mock; Phase 1 epic 1.1's ingestion sync is developed and tested against it, but does not formally `depends_on` it (dev/CI-only wiring, not a build-order dependency) |
| **2 — JOH** | `ctam-joh` | Phase 1 epics 1.2, 1.3 | needs auth + reference-data reads |
| **3 — Absence** | `ctam-absence` | Phase 2, epic 1.5 | absence hangs off JOH records; acknowledgement emails need Notification |
| **4 — Vacancy** | `ctam-vacancy` | Phase 3 | vacancies are created from absences |
| **5 — Booking** | `ctam-booking` | Phase 4 | bookings fill vacancies |
| **6 — Sitting** | `ctam-sitting` | Phase 2 | sittings are generated from JOH working patterns — **parallelisable with 4 and 5** |
| **7 — Payment** | `ctam-payment`, `ctam-payment-batch` | Phases 5 and 6, epic 1.5 | pays for bookings and sittings; the batch is the first `client_credentials` consumer |
| **8 — Itineraries** | `ctam-itinerary` | Phases 2–6 | a federated read model over everything above; owns no tables |
| **9 — MI Feed** | `ctam-mi-feed` | Phase 8 | reporting over the read model; owns no tables |
| **post-MVP — Admin UI** | `ctam-admin-ui` | Phase 1 epics 1.2, 1.3 | needs auth + reference data[^d10] |

**Parallelism worth keeping in view:** epic 1.5 (Notification) needs only the estate, so it can run alongside 1.1/1.2; and Phase 6 (Sitting) branches off Phase 2 independently of 4 and 5. Everything else in Phase 1 is close to a straight line.

## Phase 2 — JOH

### Phase 2 · Area: JOH Records & Working Patterns
...
### Phase 4 · Area: Vacancy & Cover

**Scope**: `ctam-vacancy` backend + `vacancy/` UI module. Auto-creation from approved absence with cover (R4, pre-populated with JOH type, work type, ticket, dates). Standalone vacancies. Per-day breakdown editing (cancel individual days with captured reason; extend / shorten period). `markFilled` endpoint called by Booking (Phase 5) — implemented as a direct DB UPDATE per architecture Principle 1 with explicit cross-service grants. Vacancy days locked once a booking is recorded. Fee-paid JOH filter as advertising hint (advertising itself is out-of-system; allocation decisions are recorded in CTAM by admin staff[^d12]). Cancel / close.
...
### Phase 5 · Area: Booking Management
...
### Phase 6 · Area: Sitting Management
...
> **End-of-Phase-6 demo gate**: Journey 3 (Court daily sitting confirmation) becomes demoable.

## Phase 7 — Payment

### Phase 7 · Area: Payment Processing & Reconciliation
