...
resource: 'epics/phase-0/epic-0.7-user-authenticates.html'
...
depends_on: [epic-0.0, epic-0.3]            # needs jo_people (0.3.3) + the estate
---

# Epic 0.7: User authenticates and lands on a role-scoped Home page
...
**Depends on Epic 0.3:** `jo_people` (the JOH identity-lookup target) is populated by the eLinks sync (Story 0.3.3); the shared Azure estate (AKS, PostgreSQL, ACR, APIM, App Insights) is provisioned by `ctam-reference-data` (Story 0.3.1) and consumed here.

**Vertical slice:**
- `ctam-authorisation` scaffolded from the HMCTS Crime SpringBoot template via `ctam-scaffold.sh`, following the pattern established by the first-scaffolded service (Epic 0.3, Story 0.3.1); **consumes** the shared Azure estate
...
**Out of scope (explicitly):** All upstream ingestion + `ctam-reference-data` scaffold + shared-estate provisioning + tier-(a) tables (Epic 0.3). Tier-(b) CTAM-owned reference data + the read API (Epic 0.4). FR5 machine-to-machine consumer auth (post-MVP per PRD v2.5). Real HMCTS IdP integration (mock-only at Phase 0; cuts over pre-Phase-9 per AR34). Production identity bootstrap + verification job (Epic 0.9).

---

## Story 0.7.1: Scaffold `ctam-authorisation` service from the HMCTS Crime SpringBoot template

As a **platform engineer**,
I want to scaffold `ctam-authorisation` from the HMCTS Crime SpringBoot template using `ctam-scaffold.sh`, consuming the shared Azure estate already provisioned in Epic 0.3,
...
**Given** the engineer has performed the GitHub manual-setup checklist (`ctam-architecture/runbooks/github-setup.md`) **before** running the scaffold (the canonical manual-setup pattern is established in Epic 0.3, Story 0.3.1):
...
**Given** the `ctam-architecture` Liquibase baseline changelog already created the shared `ctam_configuration_values` table ahead of `ctam-reference-data` (Epic 0.3, Story 0.3.1),
...
**And** `ctam-authorisation`'s own service-owned Liquibase changelog directory (`src/main/resources/db/changelog/`, master `db.changelog-master.yaml`) exists but is empty (auth tables created in Story 0.7.3).

**Given** the deployed service is publicly reachable through the shared APIM (provisioned in Epic 0.3),
...
## Story 0.7.2: User can authenticate against `ctam-mock-auth` and receive a JWT
...
**Given** the engineer has manually pre-created the private GitHub repo `ctam-mock-auth` with branch protection on `main` via the GitHub web UI (per `ctam-architecture/runbooks/github-setup.md`; the `gh` CLI is **not** available — see Story 0.3.1 for the canonical manual-setup pattern),
**And** runs `ctam-scaffold.sh ctam-mock-auth` (following the Story 0.3.1 pattern),
**When** the scaffold completes,
**Then** the service has the same baseline as Story 0.3.1 (Spring Boot 4.0.x, Helm chart, GitHub Actions, Actuator),
...
## Story 0.7.3: `ctam-authorisation` validates JWTs and resolves identity, roles, jurisdiction + Region/Area scope (read-only API)
...
**Given** `ctam-authorisation` is scaffolded per Story 0.7.1,
...
**And** the `ctam_authorisation` DB role owns the tables (per AR19) and holds SELECT on `jo_people` (identity lookup — `jo_people` is owned by `ctam-reference-data` and populated by the Epic 0.3 eLinks sync),
...
- The auth tables are created here; they're populated by Epic 0.9's seed scripts (dev/CI) and the production bootstrap mechanism outside the PRD's scope[^d9], not by API writes

---

## Story 0.7.4: Scaffold `ctam-ui` repo with React + TypeScript + Vite + GOV.UK base + auth wrapper
...
**References:** FR55 (foundation only — Home content populated in Story 0.7.5), FR56; NFR17, NFR18, NFR19, NFR31, NFR40; AR42–AR45b, AR53.

---

## Story 0.7.5: User signs into CTAM Pathfinder via SSO and lands on a role-scoped Home page
...
**Then** they can show a stakeholder: the platform scaffold + shared estate (Epic 0.3, Story 0.3.1), JOH reference data flowing in from eLinks (Story 0.3.3) and MRD (Story 0.8.1), the auth service scaffold (Story 0.7.1), SSO via mock-auth (Story 0.7.2), two-population authorisation enforcement (Story 0.7.3), UI foundation (Story 0.7.4), and the end-to-end sign-in flow (this story),
