...
**And** a `terraform/` directory exists with per-environment stacks (`dev` / `staging` / `production`) holding **only this service's own resources** (Key Vault namespace; the MRD storage added later in Epic 0.3, Story 0.3.1) — the shared estate lives in `ctam-shared-infrastructure` (Epic 0.0), per AR53 (revised),
...
**Given** the JOH eLinks API is unreachable or returns a structurally malformed payload (not valid JSON, or missing an entire expected entity block) mid-sync,
**When** the sync fails,
...
**Given** a payload parses as valid JSON but contains individual non-conformant records (cleansing-stage quarantine, not a structural failure),
