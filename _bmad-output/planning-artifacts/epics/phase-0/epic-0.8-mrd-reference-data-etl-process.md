...
resource: 'epics/phase-0/epic-0.8-mrd-reference-data-etl-process.html'
...
**Out of scope (explicitly):** The JOH eLinks sync (Epic 0.7). MRD API integration (post-MVP — when MRD ships public APIs). Hand-editing of `mrd_*` data in CTAM (never, in any phase — corrections at source per FR6).
