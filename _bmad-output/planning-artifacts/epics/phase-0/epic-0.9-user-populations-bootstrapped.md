...
resource: 'epics/phase-0/epic-0.9-user-populations-bootstrapped.html'
...
depends_on: [epic-0.3, epic-0.7]            # needs ctam_auth_* tables + jo_* fixtures + mock-auth roster
---

# Epic 0.9: Both user populations are bootstrapped and verifiable against the IdP

> **Renumbered from Epic 0.4** *(SCP 2026-08-25c)*: displaced to make room for the JOH data read-only API epic, which the team asked to be numbered 0.4. Content unchanged — only the number and Story 0.4.1 → 0.9.1 changed.

**User outcome:** CTAM Pathfinder's two user populations[^d9] — **JOH users** (resolved via `jo_people` → `personnel_number` → CTAM JOH UUID in `ctam_joh_identities`) and **HMCTS admin staff** (resolved via `ctam_auth_staff_identities` → CTAM-assigned UUID) — have authorisation records (roles, jurisdiction, Region/Area scope, all-FALSE activation flags) in place: seeded by scripts in dev/CI, bootstrapped by programme-management mechanisms in production (outside the PRD's scope), and **verifiable** by a bootstrap-verification job that confirms every user maps to a real IdP principal before any wave cutover. Epic 0.7's sign-in works against this data.
...
## Story 0.9.1: Identity seed scripts (both populations), bootstrap-verification job, and the production bootstrap runbook
...
So that **Epic 0.7's two-population sign-in works end-to-end in every environment, and no wave cutover can proceed with unverifiable users** (restructured D9, AR52, G1.3).
...
**And** `ctam_auth_user_activation_flags` rows keyed by (jurisdiction, region), **all FALSE** except designated test users flagged TRUE so the Epic 0.7 demo can show both the activated and non-activated paths (FR57),
...
**When** the Epic 0.7 Playwright suite executes,
**Then** the JOH test user signs in and resolves to a CTAM JOH UUID, the admin-staff test user signs in and resolves to a staff UUID (Story 0.7.5),
