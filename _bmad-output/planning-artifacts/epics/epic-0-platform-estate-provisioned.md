...
resource: 'epics/epic-0-platform-estate-provisioned.html'
...
# Epic 0: Platform estate is provisioned, verifiable, and CNP-compliant
...
**Hosting:** the shared estate lives in **`ctam-shared-infrastructure`** (CNP `{product}-shared-infrastructure` convention). This **supersedes AR53's colocated first-consumer rule** — the shared estate is no longer carried inside `ctam-reference-data/terraform/`. Each service repo's own `terraform/` continues to hold **only that service's own resources** (e.g. the MRD storage account in `ctam-reference-data`, Story 1.4).

**Why this is Epic 0 (precedes ingestion):** the shared cluster/database/registry/gateway/observability were always an implicit prerequisite of Story 1.1. Making them a first-class, independently-tested epic (a) aligns CTAM with the CNP `-shared-infrastructure` standard, (b) lets the estate be validated on its own before a service depends on it, and (c) tightens `ctam-reference-data` down to its domain — consistent with the polyrepo "minimise shared coupling" principle. The integrations-first ordering of the **domain** deliverables (1 → 5) is unchanged.
...
**Out of scope (explicitly):** any domain service scaffolding (`ctam-reference-data` — Epic 1, Story 1.1); any service's own per-repo `terraform/` resources (they stay in their service repos); production-region rollout gating (Phase 9+); the `ctam_configuration_values` Liquibase baseline (owned by `ctam-architecture`, lands ahead of Epic 1).

---

## Story 0.1: Scaffold `ctam-shared-infrastructure` and establish the Terraform foundation
...
- Any actual Azure resources — Stories 0.2–0.5
- Any service repo scaffolding — Epic 1

---

## Story 0.2: Provision networking and the AKS cluster (dev), verifiable via kubectl
...
**Given** the Terraform foundation exists per Story 0.1,
...
**Explicitly NOT in scope:**
- PostgreSQL, Key Vault, ACR, App Insights, APIM — Stories 0.3–0.5
- Deploying any CTAM service — Epic 1

---

## Story 0.3: Provision PostgreSQL and Key Vault (dev), verifiable over TLS from the cluster
...
**Given** the AKS cluster exists per Story 0.2,
...
**Explicitly NOT in scope:**
- Per-service DB roles/grants and the `ctam_configuration_values` baseline (owned by `ctam-architecture`; consumed in Epic 1)
- ACR, App Insights, APIM — Stories 0.4–0.5

---

## Story 0.4: Provision ACR and observability (dev), verifiable via image pull and a test trace
...
## Story 0.5: Provision APIM (dev), verifiable end-to-end via a smoke API over TLS
...
**And** the estate is ready for `ctam-reference-data` to scaffold and deploy onto (Epic 1, Story 1.1).
...
- Any domain service — Epic 1 onward
