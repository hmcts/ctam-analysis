...
resource: 'epics/phase-1/epic-1.5-system-dispatches-emails.html'
...
depends_on: [epic-1.0]                      # only needs the estate — parallelisable with 1.1/1.2
---

# Epic 1.5: Notification service is scaffolded and contractually ready

**User outcome:** The `ctam-notification` service is deployed with its API contract published, delivery-log table created, SMTP integration to HMCTS email infrastructure configured, and `POST /v1/notifications/send` endpoint operational. The contract is consumable from Phase 3+ (Absence acknowledgement, Booking acknowledgement) via JWT propagation — the user-initiated flow. Service-token (`client_credentials`) auth for non-user-initiated callers is **deferred to Phase 7** when `ctam-payment-batch` arrives as the first consumer. **No admin UI in MVP** — the test-send / verification flow happens via Postman during Phase 1 integration testing, not via a deployed UI.
...
- JWT propagation for user-initiated calls (per NFR12 — Phase 3+ Absence and Phase 5 Booking acknowledgement flows are user-initiated)
...
- Phase 1 integration testing via Postman: send → queued → sent → verify in Mailpit
...
**Out of scope for Phase 1 (deferred to other phases):**
- OAuth `client_credentials` flow for batch / scheduled callers — **deferred to Phase 7** (the flow is established alongside `ctam-payment-batch`, the first non-user-initiated consumer)
...
## Story 1.5.1: Scaffold `ctam-notification` service + delivery log table + SMTP integration
...
So that **downstream phases** (Phase 3 absence ack, Phase 5 booking ack, Phase 7 payment schedule) **can dispatch transactional emails** via a consistent contract from day one of consuming them.
...
**Then** the new repo has the same baseline as Stories 1.1.1 / 1.2.1 (Spring Boot 4, Helm chart, GitHub Actions, Actuator, structured logs, OpenAPI tooling, Spectral, ArchUnit, Spotless, Checkstyle, Pact, Postman),
...
## Story 1.5.2: `POST /v1/notifications/send` endpoint with JWT propagation, retry semantics, delivery logging, RFC 9457 errors

As a **calling service** (Phase 3 Absence flow, Phase 5 Booking flow — both user-initiated),
...
So that **transactional email dispatch is a single, observable, retry-safe contract** (per FR9, NFR22) that the user-initiated downstream phases consume consistently. The OAuth `client_credentials` flow for non-user-initiated callers (batch / scheduled) is **out of scope for Phase 1** — it will be added in Phase 7 when `ctam-payment-batch` arrives.

**Acceptance Criteria:**

**Given** `ctam-notification` is deployed per Story 1.5.1,
...
**Then** the request is **rejected** at Phase 1 with `403 Forbidden` and an RFC 9457 body explaining "service-principal callers are not in scope until Phase 7 (`ctam-payment-batch` arrives). Use user JWT propagation for Phase 3+ user-initiated flows.",
...
**And** the spec explicitly annotates "service-principal auth deferred to v2 (Phase 7) — Phase 1 / 2 only accept user-JWT".

**Given** a Postman collection is published,
**When** `postman/ctam-notification-phase1.postman_collection.json` runs in CI,
**Then** it covers happy path + 400 + 401 + 403 (admin-only) + 403 (service-principal rejected pre-Phase-7) + retry behaviour (via a fault-injection test endpoint, removable post-Phase-1),
**And** Phase 1 manual integration test consists of: open Postman → authenticate as test user → POST send → poll delivery-log until `sent` → open Mailpit → verify rendered email. (No admin UI for this flow in MVP.)
...
- OAuth `client_credentials` flow for batch / scheduled callers — **Phase 7**, alongside `ctam-payment-batch`
