...
resource: 'epics/phase-1/epic-1.1-upstream-reference-data-ingested.html'
...
**Out of scope (explicitly):** the read-only Reference Data API + jurisdiction filtering (Epic 1.3, Story 1.3.2 — downstream of auth). Tier-(b) CTAM-owned reference tables (Epic 1.3, Story 1.3.1). All authentication / authorisation / UI (Epic 1.2). MRD API integration (post-MVP — when MRD ships public APIs). Hand-editing of tier-(a) data in CTAM (never, in any phase — corrections at source per FR6).
...
**And** a `terraform/` directory exists with per-environment stacks (`dev` / `staging` / `production`) holding **only this service's own resources** (Key Vault namespace; the MRD storage added in Story 1.1.4) — the shared estate lives in `ctam-shared-infrastructure` (Epic 1.0), per AR53 (revised),
...
**And** the deployment fails fast with a clear diagnostic if any Epic 1.0 estate dependency is absent (making the Epic 1.0 → 1.1 sequencing explicit).
...
**References:** FR8, FR58, FR59; NFR10, NFR11, NFR15, NFR16, NFR24, NFR25–NFR28, NFR31, NFR40, NFR42; AR2–AR17, AR23–AR32, AR41, AR53 (revised — estate provisioned in Epic 1.0); **D10** (`gh` CLI not available — manual GitHub web-UI setup per `ctam-architecture/runbooks/github-setup.md`); **depends on Epic 1.0** (shared estate).
...
- Tier-(a) `jo_*` / `mrd_*` tables and `ctam_sync_status` — Story 1.1.2
- The eLinks sync and MRD ingestion mechanisms — Stories 1.1.3 / 1.1.4
- Tier-(b) CTAM-owned tables + the read-only API — Epic 1.3
...
**And** SELECT grants exist for `ctam_authorisation` (identity lookup, Epic 1.2) and placeholder roles for future services,
...
**Explicitly NOT in scope:**
- The eLinks sync that populates these tables — Story 1.1.3
- The `mrd_*` tables — Story 1.1.4
- Tier-(b) CTAM-owned tables (regions, offices, vocabularies) — Epic 1.3, Story 1.3.1
- The read-only REST API — Epic 1.3, Story 1.3.2

---

## Story 1.1.3: JOH reference data flows into CTAM nightly from the JOH eLinks API
...
**Given** the tier-(a) `jo_*` tables and `ctam_sync_status` exist per Story 1.1.2,
...
**When** `ctam-authorisation` (Epic 1.2, Story 1.2.3) looks up a seeded JOH email,
...
**Explicitly NOT in scope:**
- MRD ingestion — Story 1.1.4
- The read-only REST API — Epic 1.3, Story 1.3.2

---

## Story 1.1.4: MRD supplementary reference data is ingested from the weekly Excel feed
