...
>
> The Phase 1 areas in [../framework.md](../framework.md) are an **architectural map**. The ten concrete epics below are the **implementation plan** — each delivers a demoable outcome. Stories are not yet written for this phase (reverted 2026-08-21, SCP 2026-08-21e) except Epic 1.9's Story 1.9.1, a completion record for genuinely finished work.
...
> Reverted to epic-level-only 2026-08-21 (SCP 2026-08-21e, direct user request) — stories were written and later business-language-rewritten, then stripped back out, leaving these epics in the state `bmad-create-epics-and-stories` step 2 produces (epics designed, not yet decomposed into stories). One exception: Epic 1.9's Story 1.9.1 is kept, since it documents real, already-published work (see its epic file).

| Epic | Title | Stories | Status |
|---|---|---|---|
| [1.0](epic-1.0-platform-estate-provisioned.md) | Platform estate is provisioned, verifiable, and CNP-compliant | 0 | 🟡 Planned |
| [1.1](epic-1.1-postgres-db-schema-design.md) | Postgres reference-data schema is designed and scaffolded | 0 | 🟡 Planned |
| [1.2](epic-1.2-joh-reference-data-etl-process.md) | JOH reference data flows into CTAM via the nightly eLinks ETL process | 0 | 🟡 Planned |
| [1.3](epic-1.3-mrd-reference-data-etl-process.md) | MRD reference data flows into CTAM via the weekly Excel ETL process | 0 | 🟡 Planned |
| [1.4](epic-1.4-user-authenticates.md) | User authenticates and lands on a role-scoped Home page | 0 | 🟡 Planned |
| [1.5](epic-1.5-joh-reference-data-read-api.md) | JOH and CTAM-owned reference data is served read-only via a versioned, jurisdiction-filtered API | 0 | 🟡 Planned |
| [1.6](epic-1.6-mrd-reference-data-read-api.md) | MRD reference data is served read-only via the Reference Data API | 0 | 🟡 Planned |
| [1.7](epic-1.7-user-populations-bootstrapped.md) | Both user populations are bootstrapped and verifiable against the IdP | 0 | 🟡 Planned |
| [1.8](epic-1.8-system-dispatches-emails.md) | Notification service is scaffolded and contractually ready | 0 | 🟡 Planned |
| [1.9](epic-1.9-context-bus-and-shared-baseline.md) | Context bus is published and the shared configuration baseline exists | 1 | 🟢 In progress |
| **Total** | | **1 story** | |
...
**Outcome:** The shared Azure environment — the Kubernetes cluster, database, container registry, API gateway, monitoring, and secrets vault — is stood up in its own dedicated setup, provisioned one layer at a time with each layer proven working before the next is built, so every Phase 1 service has a tested platform to deploy onto. Comes before Epic 1.1. Planned as five stages, in order: the repository and provisioning foundation; the network and cluster; the database and secrets vault; the container registry and monitoring; and the API gateway with an end-to-end smoke test — not yet decomposed into stories.

→ [Full epic](epic-1.0-platform-estate-provisioned.md)

### Epic 1.1: Postgres reference-data schema is designed and scaffolded

**Outcome:** The reference-data service is the first service scaffolded and deploys onto the shared platform from Epic 1.0; the database design for the data coming in from outside sources — the JOH tables and a sync-tracking table — is put in place with clear rules about who's allowed to write to it. This is the platform's foundational data layer; the JOH and MRD data-import work that follows (Epics 1.2 and 1.3) populates it once it exists.

→ [Full epic](epic-1.1-postgres-db-schema-design.md)
...
→ [Full epic](epic-1.4-user-authenticates.md)

### Epic 1.5: JOH and CTAM-owned reference data is served read-only via a versioned, jurisdiction-filtered API

**Outcome:** CTAM's own reference data (regions, offices, calendar, operational vocabularies) is created, seeded, and maintained directly by the database team for now; together with the JOH data from Epic 1.2, it's made available through a versioned, read-only, jurisdiction-aware API. Comes after Epic 1.4, since it needs sign-in and permission-checking to already exist. The MRD side of this same API is a separate epic (1.6), which extends this work rather than duplicating it.

→ [Full epic](epic-1.5-joh-reference-data-read-api.md)
...
→ [Full epic](epic-1.6-mrd-reference-data-read-api.md)
...
→ [Full epic](epic-1.7-user-populations-bootstrapped.md)
...
→ [Full epic](epic-1.8-system-dispatches-emails.md)

### Epic 1.9: Context bus is published and the shared configuration baseline exists (1 story)

**Outcome:** Every service repository can pin one published, versioned copy of the shared architecture, and every service can read cross-service policy values from one shared table it doesn't own. Both exist before the first domain service is scaffolded. **Story 1.9.1 (publishing the architecture package) is done — genuinely completed and kept in this epic's file as a completion record.** The second piece of work (creating the shared configuration table) is not yet decomposed into a story.
