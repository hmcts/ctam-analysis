...
resource: 'epics/epic-1-upstream-reference-data-ingested.html'
...
- **Consumes the shared Azure estate** (AKS, shared global PostgreSQL Flexible Server + per-service DB roles, ACR, APIM, App Insights, Key Vault) **provisioned in Epic 0** per AR53 (revised — dedicated `ctam-shared-infrastructure`)
...
**Out of scope (explicitly):** the read-only Reference Data API + jurisdiction filtering (Epic 3, Story 3.2 — downstream of auth). Tier-(b) CTAM-owned reference tables (Epic 3, Story 3.1). All authentication / authorisation / UI (Epic 2). MRD API integration (post-MVP — when MRD ships public APIs). Hand-editing of tier-(a) data in CTAM (never, in any phase — corrections at source per FR6).

---

## Story 1.1: Scaffold `ctam-reference-data` from the HMCTS starter (onto the Epic 0 estate)

As a **platform engineer**,
I want to scaffold the **first** CTAM Pathfinder backend service — `ctam-reference-data` — from the HMCTS Crime SpringBoot template using `ctam-scaffold.sh`, and to deploy it onto the shared Azure estate provisioned in Epic 0,
...
**And** a `terraform/` directory exists with per-environment stacks (`dev` / `staging` / `production`) holding **only this service's own resources** (Key Vault namespace; the MRD storage added in Story 1.4) — the shared estate lives in `ctam-shared-infrastructure` (Epic 0), per AR53 (revised),
...
**Given** the shared Azure estate has been provisioned and independently verified in **Epic 0** (`ctam-shared-infrastructure`) — AKS, PostgreSQL Flexible Server, ACR, APIM + base policies, Application Insights / Log Analytics, Key Vault — all in UK South with the documented SKUs (per A34, gaps.md G9),
**When** `ctam-reference-data`'s Helm chart is deployed to the dev AKS cluster,
**Then** the service reaches the shared cluster, database, registry, gateway, and observability estate provisioned in Epic 0 (this story **consumes** the estate; it does not provision it — AR53 revised),
**And** the deployment fails fast with a clear diagnostic if any Epic 0 estate dependency is absent (making the Epic 0 → 1 sequencing explicit).
...
**And** `ctam-reference-data`'s own service-owned Liquibase changelog directory (`src/main/resources/db/changelog/`, master `db.changelog-master.yaml`) exists but is empty (tier-(a) tables created in Story 1.2).

**Given** the shared APIM gateway (provisioned + TLS-verified in Epic 0, Story 0.5),
**When** `ctam-reference-data` registers its API through APIM and an HTTP request reaches the gateway,
**Then** the service's API is reachable over TLS (the gateway TLS floor itself is verified in Epic 0 per NFR10),
**And** HTTP-only requests are rejected with a redirect to HTTPS.

**Given** the shared PostgreSQL Flexible Server (provisioned + encryption/TLS-verified in Epic 0, Story 0.3),
**When** the Helm chart's `values-dev.yaml` is applied,
**Then** the database connection string references the shared Epic 0 PostgreSQL instance (storage-encrypted at rest per NFR11; TLS-only per NFR10 — both verified in Epic 0),
**And** `ctam-reference-data` connects successfully using its own DB role.

**Given** the shared Application Insights workspace (provisioned in Epic 0, Story 0.4, with the agreed retention policy),
**When** the deployed service emits telemetry,
**Then** `ctam-reference-data`'s structured logs and traces land in the shared workspace (retention is owned by Epic 0 per NFR26).
...
**References:** FR8, FR58, FR59; NFR10, NFR11, NFR15, NFR16, NFR24, NFR25–NFR28, NFR31, NFR40, NFR42; AR2–AR17, AR23–AR32, AR41, AR53 (revised — estate provisioned in Epic 0); **D10** (`gh` CLI not available — manual GitHub web-UI setup per `ctam-architecture/runbooks/github-setup.md`); **depends on Epic 0** (shared estate).
...
- Tier-(a) `jo_*` / `mrd_*` tables and `ctam_sync_status` — Story 1.2
- The eLinks sync and MRD ingestion mechanisms — Stories 1.3 / 1.4
- Tier-(b) CTAM-owned tables + the read-only API — Epic 3

---

## Story 1.2: Tier-(a) upstream `jo_*` tables, `ctam_sync_status`, and tier-(a) write protection
...
**Given** `ctam-reference-data` is scaffolded per Story 1.1,
...
**And** SELECT grants exist for `ctam_authorisation` (identity lookup, Epic 2) and placeholder roles for future services,
...
**Explicitly NOT in scope:**
- The eLinks sync that populates these tables — Story 1.3
- The `mrd_*` tables — Story 1.4
- Tier-(b) CTAM-owned tables (regions, offices, vocabularies) — Epic 3, Story 3.1
- The read-only REST API — Epic 3, Story 3.2

---

## Story 1.3: JOH reference data flows into CTAM nightly from the JOH eLinks API
...
**Given** the tier-(a) `jo_*` tables and `ctam_sync_status` exist per Story 1.2,
...
**When** `ctam-authorisation` (Epic 2, Story 2.3) looks up a seeded JOH email,
...
**Explicitly NOT in scope:**
- MRD ingestion — Story 1.4
- The read-only REST API — Epic 3, Story 3.2

---

## Story 1.4: MRD supplementary reference data is ingested from the weekly Excel feed
