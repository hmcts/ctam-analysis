...
resource: 'implementation-readiness-report-2026-05-05.html'
...
**Date:** 2026-05-05
**Project:** ctam-analysis (CTAM Pathfinder — CTAM Pathfinder)
...
>
> 0. **FR59 / NFR39 / API-as-Product (v2.7, 2026-05-08)** — RFC citations refreshed: error envelope now [RFC 9457](https://datatracker.ietf.org/doc/html/rfc9457) (obsoletes RFC 7807 — content unchanged); deprecation signalling cited correctly to [RFC 9745](https://datatracker.ietf.org/doc/html/rfc9745) `Deprecation` + [RFC 8594](https://datatracker.ietf.org/doc/html/rfc8594) `Sunset`.
>
> 1. **FR41–FR45 (Payment)** — reframed as a **scheduled batch flow** (`ctam-payment-batch`), not user-initiated. RSU/Sam confirms bookings/sittings; the batch picks up confirmed-but-unpaid records on its next scheduled run, generates the JFEPS Excel, and dispatches it to the Payment Authoriser. See `architecture/sequence-diagrams/payment-batch-flow.md`.
> 2. **NFR12 / Service-to-service auth** — v2.5 narrowed this to "JWT propagation only, no service principals." v2.6 widened it back: **two patterns at MVP** — JWT propagation for user-initiated flows AND OAuth `client_credentials` (against `ctam-mock-auth` in non-prod) for the payment batch. Production issuer per `gaps.md` G7.1 (default recommendation: Azure Workload Identity).
>
...
- **FR1**: Authenticated users access CTAM Pathfinder via HMCTS IdP single sign-on; password, session, and account lifecycle are owned by the IdP and not duplicated in CTAM Pathfinder.
...
- **FR7**: Every CTAM Pathfinder service can read Reference Data via a versioned API; Reference Data is the single writer (no duplicates anywhere in the system).
- **FR8** *(revised v2.2, 2026-05-07)*: Cross-service runtime policy values (e.g. session timeout warnings, batch schedules, feature flags) live in a shared `configuration_values` infrastructure table, schema-managed by `ctam-architecture`'s Flyway baseline migration. Updates via Flyway / admin SQL — no API service. Per-service config that's scoped to one service uses Spring profiles + Key Vault.
...
- **FR13**: CTAM Pathfinder auto-populates judge itineraries up to the next 31st March from the working pattern, preserving any prior absences.
...
- **FR27**: CTAM Pathfinder surfaces fee-paid judges matching a vacancy's filter as a hint for advertising; advertising itself is performed out-of-system by judicial teams.
...
- **FR32**: CTAM Pathfinder sends booking acknowledgement emails to fee-paid judges, batched overnight or sent immediately via *Create and Email Now*.
...
- **FR43**: CTAM Pathfinder generates JFEPS-compatible payment schedules and dispatches them as Excel attachments to a chosen Payment Authoriser via email; the Payment Authoriser forwards to Liberata out-of-system.
...
- **FR45**: CTAM Pathfinder prevents double submission of the same booking for payment.
...
- **FR56**: CTAM Pathfinder's UI replicates the functional surface of the as-is APEX UI on a modern UI stack and meets WCAG 2.2 Level AA accessibility standards.
- **FR57** *(reframed 2026-05-06)*: A Phase 0 Data Migration ETL takes Reference Data and active user records from APEX (legacy schema), transforms them into CTAM Pathfinder's own (independently-designed) shape, and loads them via the CTAM Pathfinder Reference Data API and Authorisation API. Migrated user records are keyed to HMCTS IdP principals (email primary, employee number fallback). Phase 0 deliverable with named-owner sign-off; unmatched user records flagged for explicit handling (drop / hold / manual map). The ETL is *not* Flyway data-seeding — it's a separate programme deliverable at `ctam-architecture/migration/`.
- **FR58**: CTAM Pathfinder supports per-region phased activation — a region's user accounts can be activated for CTAM Pathfinder use only when that region's feature-parity gate is passed; activation is a flag flip, not a data migration.
- **FR59**: Every CTAM Pathfinder service exposes a versioned API contract, RFC 7807 problem-details for errors, and a published OpenAPI specification. *(Note: revised v2.7, 2026-05-08 — RFC 7807 reference updated to [RFC 9457](https://datatracker.ietf.org/doc/html/rfc9457). This snapshot preserves the wording at 2026-05-05 with the RFC pointer brought current.)*
...
- **FR61**: Every CTAM Pathfinder domain service has a manual user acceptance test (UAT) script for APEX-experienced users to walk through, comparing CTAM Pathfinder vs APEX side-by-side; sign-off per role per region is the wave-cutover gate. There is no automated APEX-comparison harness. *(Revised 2026-05-06; supersedes earlier wording about real APEX running as an automated comparison reference in CI. See architecture changelog v1.7.)*
...
- **NFR35**: Zero failed JFEPS payment cycles attributable to CTAM Pathfinder.
...
- **NFR39**: API-as-Product standards (versioning, RFC 7807, OpenAPI). *(Note: revised v2.7, 2026-05-08 — RFC 7807 → [RFC 9457](https://datatracker.ietf.org/doc/html/rfc9457). This snapshot preserves the wording at 2026-05-05.)*
- **NFR40**: Per-service deployment unit on Kubernetes; independently deployable.
- **NFR41**: Manual UAT script per domain service (revised 2026-05-06) — APEX-experienced users compare CTAM Pathfinder vs APEX side-by-side; sign-off per role per region is the wave gate. No automated parity test suite.
...
- **Capability coverage:** 61 FRs across 9 areas covering all 11 services in the locked decomposition (revised v2.2, 2026-05-07 — `ctam-configuration` dropped). Cross-checked in Step 11 polish against the brainstorming source — nothing dropped.
...
- **Phase 0 cross-cutting epics** (Identity & Authorisation, Reference Data, API Platform) tend to look like technical milestones. They can be framed as user-value (e.g. "RSU users can log in to CTAM Pathfinder via SSO and see their authorised regions" rather than "Setup SSO"). Frame them as user-facing capabilities even when their consumers are internal cross-cutting concerns.
...
- **Behavioural-parity UAT stories** (FR61 / NFR41 revised 2026-05-06) should appear within each domain epic, not as a separate technical epic. UAT script authoring (under `docs/uat/`) and per-wave UAT execution are owned by the service being tested. UAT is performed manually by APEX-experienced users (RSU, Court, Judge, Clerks, Finance, MI) comparing CTAM Pathfinder vs APEX side-by-side; sign-off per role per region is the wave-cutover gate. There is no automated APEX-comparison test work to schedule.
...
- **Capability completeness:** 61 FRs across 9 capability areas covering all 11 services (revised v2.2 — `ctam-configuration` dropped); brainstorming reconciliation completed in PRD Step 11 found nothing dropped.
