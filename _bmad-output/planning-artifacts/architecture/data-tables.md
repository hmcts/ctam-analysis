...
ListAssist and APEX have their own (different) schemas; neither is in this inventory and neither is owned by any CTAM Pathfinder service. **No legacy data is migrated from either system**[^d3] — the Phase 0 Data Migration ETL is retracted. Judicial-holder reference data is ingested from the upstream sources of truth:
...
| `ctam_payment_schedules` | Domain | JFEPS-shaped schedule snapshots (FR43, FR44). JFEPS format preserved for SSCS wave 1[^d11]. |
