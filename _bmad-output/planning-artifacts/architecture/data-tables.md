...
The incumbent scheduling systems — `[ET-INCUMBENT-TBD]` (ET wave 1, unidentified per G8.4), ListAssist (SSCS wave 2) and APEX (Courts waves 3+) — have their own (different) schemas; none is in this inventory and none is owned by any CTAM Pathfinder service. **No legacy data is migrated from any of them**[^d3][^d13] — the Phase 1 Data Migration ETL is retracted. Judicial-holder reference data is ingested from the upstream sources of truth:
