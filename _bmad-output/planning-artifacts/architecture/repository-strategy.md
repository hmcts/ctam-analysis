...
| **`ram-joh`** | 1 | JOH operational state — working patterns + ticket/location overlays + jurisdictional split, keyed by `joh_id` → `ram_joh_identities`. *(Renamed from `ram-joh`[^d11]; the canonical JOH person record is `jo_people`, owned by Reference Data.)* | Manage working patterns (FR12); generate forward sittings (FR13); manage ticket overlays (FR15b) and location overlays (FR17); jurisdictional splits with 100% sum constraint (FR16); JOH search/profile views compose `jo_*` + overlays (FR10, FR11). |
...
**16 repos total** (11 production services + 2 UI + architecture + mock-auth + shared-infrastructure). `ram-shared-infrastructure` is Terraform-only (no deployable workload); `ram-mock-auth` is dev/integration-only and never deploys to production.
