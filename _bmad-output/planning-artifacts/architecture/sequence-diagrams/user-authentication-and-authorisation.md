...
description: 'Sequence diagram of the cross-cutting authentication and per-request authorisation flow that underpins every user-initiated interaction in CTAM Pathfinder.'
...
Sequence diagram of the cross-cutting authentication and per-request authorisation flow that underpins every user-initiated interaction in CTAM Pathfinder. A user authenticates once at the HMCTS IdP (or `ctam-mock-auth` in non-prod); the issued JWT is then forwarded by the UI on every subsequent API call. Each backend service's custom `JWTFilter` validates the token against the issuer's JWKS endpoint and asks `ctam-authorisation` to resolve the principal's identity and scope before the controller is allowed to run. Per the restructured D9 (2026-06-10), identity resolution is **two-population**: a JOH user's IdP email is looked up against `jo_people` to resolve the `personnel_number`, which maps (via `ctam_joh_identities`) to the **CTAM JOH UUID**; an HMCTS admin-staff user's email is looked up against `ctam_auth_staff_identities` to resolve the CTAM-assigned UUID. The authz result carries roles + **jurisdiction** + Region/Area scope + the (jurisdiction, region) activation flag (FR57).
...
- **Service-principal authentication** for batch components — `ctam-payment-batch` uses OAuth `client_credentials` rather than the human SSO flow. See [`./payment-batch-flow.md`](./payment-batch-flow.md) Phase 1.
...
- **Token refresh / expiry handling** in the UI — standard OIDC behaviour; not CTAM Pathfinder-specific.
...
- The two UIs (`ctam-ui` and `ctam-admin-ui`) follow the same auth pattern. The diagram uses "UI" generically.
...
| 3 — Per-request authz check | Any backend service | `JWTFilter` calls `ctam-authorisation` `POST /authz/check`; resolves IdP email → canonical identifier (CTAM JOH UUID via `jo_people` → `personnel_number` → `ctam_joh_identities` for JOHs; CTAM UUID via `ctam_auth_staff_identities` for admin staff) → roles + jurisdiction + Region/Area scope + `ctam_auth_user_activation_flags` (FR57); populates request-scoped `AuthDetails` bean | Controller method invoked with authorised principal in scope; or 403 if unresolvable or not active for the (jurisdiction, region) |
...
| `JWTFilter` per-service implementation pattern and `AuthDetails` request-scoped bean | [`../starter-template.md` → Per-service CTAM Pathfinder Conventions](../starter-template.md); [`../repo-structure.md` per-service `config/JWTFilter.java`, `config/AuthDetails.java`](../repo-structure.md) |
| Authorisation tables (`ctam_auth_users`, `ctam_auth_staff_identities`, `ctam_auth_roles`, `ctam_auth_user_roles`, `ctam_auth_user_region_scopes`, `ctam_auth_user_activation_flags`) + the `jo_people` identity lookup | [`../data-tables.md`](../data-tables.md) |
...
| Two-UI repo split (`ctam-admin-ui` post-MVP[^d10]; same auth pattern) | [`../../architecture.md` → Step 4 *Frontend Architecture*](../../architecture.md) — v2.10 / D10 |
