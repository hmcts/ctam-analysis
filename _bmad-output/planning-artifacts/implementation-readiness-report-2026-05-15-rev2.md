...
**Date:** 2026-05-15
**Project:** ctam-analysis (CTAM Pathfinder rebuild)
...
- **MVP scope bullet 3** distinguishes business UI (in MVP via `ctam-ui`) vs admin UI (post-MVP per D10).
...
| **FR4** | 0.3.1 (data layer via SQL ETL into `auth_users`/`auth_user_roles`/`auth_user_region_scopes`) | ✅ data layer (DBAs edit via SQL) | UI surface for sysadmins (`ctam-admin-ui` Users & Roles module) | ✅ correctly scoped per D10 — NOT a coverage gap |
...
| FR55 | 0.1.5 | ✅ full | — | ✅ |
| **FR56** | 0.1.4 (`ctam-ui` scaffold), 0.1.5 (Home shell), 0.2.2 (consumes API) | ✅ business stack | `ctam-admin-ui` admin stack post-MVP per D10 | ✅ correctly scoped — NOT a coverage gap |
...
- **Business UI** (`ctam-ui`) — in MVP via D4 (modern stack replicates APEX), GOV.UK Design System, WCAG 2.2 AA per NFR17–NFR19. No UX doc; APEX is the behavioural reference.
- **Admin UI** (`ctam-admin-ui`) — post-MVP per D10. A UX design will be needed when the admin UI is scheduled. Out of MVP scope, no gap.
...
2. **Implement Story 0.1.1 first** — produces the canonical platform pattern AND the `ctam-architecture/runbooks/github-setup.md` deliverable that the other scaffold stories depend on.
