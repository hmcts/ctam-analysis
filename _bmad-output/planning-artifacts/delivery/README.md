...
| Which **repo** an epic lands in | the epic's frontmatter (`repo:`) | May be a list — Epic 2 spans three repos |
...
Not here, deliberately: the **context bus** (`ctam-architecture` publish + `arch-vN` tag) and **per-repo scaffolding** — those are [Epic 6](../epics/epic-6-context-bus-and-shared-baseline.md) and downstream work. The control plane only ever *references* a `bus_version` as a string.
...
**Buildable-now rule:** an epic is dispatchable iff every epic in its `depends_on` is `done` in `sprint-status.yaml`. Epics with disjoint dependency sets and no shared state may run **in parallel** — Epic 5 needs only the estate, so it can run alongside 1/2.
...
- **Phase 0:** 7 epics, 21 stories in `sprint-status.yaml`. Epic 6 is `in-progress` (story 6.1 — the bus published and tagged `arch-v1.0` — is `done`); everything else is `backlog`.
