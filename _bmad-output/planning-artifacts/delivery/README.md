...
| Which **repo** an epic lands in | the epic's frontmatter (`repo:`) | May be a list — Epic 0.4 spans three repos |
| Build **order** | the epic's frontmatter (`depends_on:`) | Phase 0 is structured; phases 1–8 are prose in [`../epics/framework.md`](../epics/framework.md) → *Phase dependency order* until they are decomposed |
...
Not here, deliberately: the **context bus** (`ctam-architecture` publish + `arch-vN` tag) and **per-repo scaffolding** — those are [Epic 0.9](../epics/phase-0/epic-0.9-context-bus-and-shared-baseline.md) and downstream work. The control plane only ever *references* a `bus_version` as a string.
...
**Buildable-now rule:** an epic is dispatchable iff every epic in its `depends_on` is `done` in `sprint-status.yaml`. Epics with disjoint dependency sets and no shared state may run **in parallel** — Epic 0.8 (Notification) needs only the estate, so it can run alongside 0.1/0.2/0.3/0.4.
