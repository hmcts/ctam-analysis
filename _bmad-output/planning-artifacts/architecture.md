...
- **JOH eLinks API** — canonical upstream source for the 15 `jo_*` judicial-holder entities[^d3]. Pulled by `ctam-reference-data`'s scheduled sync; read-only in CTAM; corrections at source (Judicial Office). No data flows upstream from CTAM. **Contract unconfirmed (G8.1); dev/CI/integration stand-in is `ctam-joh-mock`** (AR55) — the cutover to real eLinks is a configuration change.
- **MRD (Master Reference Data)** — supplementary judicial reference data (notably JOH Specialisations). Weekly Excel feed for MVP (public MRD APIs not yet available); transitions to API integration when MRD APIs ship. **Delivery arrangements unconfirmed (G8.1); dev/CI/integration stand-in is `ctam-mrd-mock`** (AR56) — the cutover changes only who drops into the container.
...
| 14 | Network isolation + edge protections added to the platform estate *(SCP 2026-08-14)* | **New Story 0.0.6** hardens the shared estate after Stories 0.0.2–0.0.5: NSGs on every AKS subnet, private-endpoint-only access for PostgreSQL/Key Vault/ACR (public network access disabled), and a WAF (OWASP CRS) + DDoS Protection policy at the APIM edge. Closes the gap between **NFR15** (GovS7 alignment) and Epic 0.0's acceptance criteria, which previously exercised TLS/encryption-at-rest/secret-management (NFR10/NFR11/NFR16) but no network-perimeter control. **New AR54**; **new gap G10** (DDoS tier + WAF policy ownership; cross-repo private-DNS-zone conventions). Phase 0: 19 → 20 stories (Epic 0.0: 5 → 6 stories). **No code impact** (implementation not started). See [`../sprint-change-proposal-2026-08-14.md`](../sprint-change-proposal-2026-08-14.md). |
| 15 | Postgres schema design split out as its own epic; JOH/MRD ingestion split into two epics; Phase 0 renumbered *(SCP 2026-08-15)* | **New Epic 0.1** ("Shared PostgreSQL schema design is established and CI-enforced") finalizes the relational schema in `architecture/data-tables.md` and builds a CI fitness function enforcing naming/PK/FK/timestamp/tier-prefix conventions, **before** any table-creating epic. **The former combined ingestion epic (JOH + MRD) splits into two independently-demoable epics**: JOH eLinks (JSON, nightly) and MRD Excel (weekly Blob drop) are a different format, cadence, and delivery mechanism each. **Phase 0 is fully renumbered** to make room: 0.0 platform estate (unchanged) → **0.1 schema design (new)** → **0.2 JOH ingestion** (was 0.1, trimmed to its 3 JOH-only stories) → **0.3 MRD ingestion (new, 1 story, was 0.1's Story 0.1.4)** → **0.4 auth** (was 0.2) → **0.5 read API** (was 0.3, now depends on both 0.2 and 0.3 for tier-(a) data) → **0.6 bootstrap** (was 0.4) → **0.7 notification** (was 0.5). Every epic file, ledger shard, and the dispatch graph is renamed/renumbered accordingly. Phase 0: 6 → 8 epics, 20 → 22 stories. **No code impact** (implementation not started). See [`../sprint-change-proposal-2026-08-15b.md`](../sprint-change-proposal-2026-08-15b.md). |
| 16 | Reference Data read-only API split into a JOH+tier-(b) surface and an MRD surface *(SCP 2026-08-15c)* | **Epic 0.5** ("JOH and tier-(b) reference data served read-only...") is trimmed to serve only JOH tier-(a) data + the 15 tier-(b) CTAM-owned tables (none of which are MRD-sourced). **New Epic 0.6** ("MRD reference data served read-only...") gives MRD Specialisations its own read-API surface — same deployable (`ctam-reference-data`), same OpenAPI artefact, a distinct resource group — mirroring decision #15's ingestion split at the read-API layer. **Phase 0 renumbered again**: 0.6 bootstrap → **0.7**, 0.7 notification → **0.8**. Dependency graph updated: Epic 0.5 drops its Epic 0.3 (MRD) dependency; new Epic 0.6 depends on Epic 0.1 (schema) + Epic 0.3 (MRD data) + Epic 0.4 (auth). Phase 0: 8 → 9 epics, 22 → 23 stories. **No code impact** (implementation not started). See [`../sprint-change-proposal-2026-08-15c.md`](../sprint-change-proposal-2026-08-15c.md). |
| 17 | Both upstream sources get contract-shaped mock repos, built before their ingestion epics *(SCP 2026-08-18b; **placement and numbering superseded by #18**)* | **Neither upstream contract is confirmed** (gaps.md G8.1, a wave-1 blocker), yet Epic 0.2 (nightly eLinks sync) and Epic 0.3 (weekly MRD workbook) cannot be built, demoed, or negative-tested without an upstream to talk to. **Two new repos** — **`ctam-joh-mock`** (AR55: all 15 `jo_*` entities as JSON in CTAM's expected eLinks contract shape, version-tagged ET-flavoured fixtures carrying the Employment Tribunals jurisdiction hierarchy, fault-injection modes) and **`ctam-mrd-mock`** (AR56: MRD-shaped workbook generator publishing conformant and deliberately-malformed variants into a configurable blob container, Azurite in CI) — delivered as **new Epics 0.9 and 0.10**, each sequenced **before** the ingestion epic that consumes it. This generalises the `ctam-mock-auth` mock-first pattern from identity to the upstream data plane: an external-team dependency becomes a **configuration cutover**. Both are dev/CI/integration-only, never deployed to production, guarded three ways (production-profile refusal, pipeline exclusion, CI lint) per G5.3 as widened; neither owns a table in the shared schema. **Numbering:** the epics were **appended as 0.9/0.10 rather than inserted at 0.2/0.3**, so 23 already-written stories did not renumber — an epic number is now explicitly an **identifier, not a build position**, and `delivery/dispatch-graph.yaml` is the sole authority on order (topological order: `0.0 → arch-baseline → 0.1 → 0.9 → 0.2 → 0.10 → 0.3 → 0.4 → 0.5 → 0.6 → 0.7 → 0.8`). **Repos: 16 → 18. Phase 0: 9 → 11 epics, 23 → 27 stories.** G8.1 is **de-risked but explicitly not closed** — the mocks encode CTAM's written expectation, not evidence about the real upstreams. **No code impact** (implementation not started). See [`../sprint-change-proposal-2026-08-18b.md`](../sprint-change-proposal-2026-08-18b.md). **Superseded in part by #18 (SCP 2026-08-18c):** the two epics moved to their own pre-Phase-0 tier, **Phase 0-Mock**, and were renumbered **0.9 → 0M.1** and **0.10 → 0M.2**. The counts stated above ("Phase 0: 9 → 11 epics, 23 → 27 stories") and the append-rather-than-insert numbering rationale are the record of *this* decision and no longer describe the current state — Phase 0 is 9 epics / 23 stories and Phase 0-Mock is 2 epics / 4 stories. Everything else in #17 — the two repos, AR55/AR56, the G5.3 widening, the G8.1 de-risked-not-closed position, and the mock-first rationale — stands. |
| 18 | Upstream stand-ins promoted to their own pre-Phase-0 tier, **Phase 0-Mock** *(SCP 2026-08-18c)* | **Supersedes #17 on placement and numbering only.** The two stand-in epics move out of Phase 0 into a **new phase folder `epics/phase-0-mock/`** with its own phase index, its own row in the epics phase table and in `epics/framework.md`, and its own site-NAV group — and are renumbered **`0.9` → `0M.1`** (`ctam-joh-mock`) and **`0.10` → `0M.2`** (`ctam-mrd-mock`), with story ids `0M.1.1`/`0M.1.2`/`0M.2.1`/`0M.2.2`. **Why:** #17 appended the epics at 0.9/0.10 to avoid renumbering 23 stories, and paid for it with a standing trap — two epics whose numbers sort *after* Epic 0.8 but which must build *before* Epic 0.2 — mitigated only by warning notes in five places. Giving the tier its own phase and an `0M.` prefix removes the trap at its source: the identifier now carries the sequencing information, so no note is needed to prevent the misreading. It also states a real structural fact that #17 obscured: the stand-in work is **not** foundations, it is the scaffolding that makes foundations buildable, and it is the only work in the programme whose sole purpose is to be thrown into contact with a contract nobody has seen. **Phase 0-Mock is explicitly not a slot in the 0–9 phase sequence** — its epics are dependencies *of* Epics 0.2 and 0.3 and nothing else depends on them. **Counts: Phase 0-Mock 2 epics / 4 stories; Phase 0 reverts to 9 epics / 23 stories.** Build order remains a **partial** order — both `0M.` epics become dispatchable as soon as `epic-0.1` is `done`, do not wait on each other, and are parallelisable with `epic-0.8`; `epic-0M.1` gates `epic-0.2` and `epic-0M.2` gates `epic-0.3`. Graph re-verified acyclic. **Unchanged:** both repos and their scope, AR55/AR56, G5.3 as widened, G8.1's de-risked-but-open position, the 18-repo count, and `ctam-mock-auth`'s placement as Story 0.4.2 inside Epic 0.4 (it is an identity-provider mock inside a working auth slice, not an upstream data-source mock). **No FR, NFR or code impact** (implementation not started). See [`../sprint-change-proposal-2026-08-18c.md`](../sprint-change-proposal-2026-08-18c.md). |
| 19 | Existing eLinks v5 mock adopted brownfield; Node/Express accepted as the one AR2–AR17 exception *(SCP 2026-08-18d)* | **Epic 0M.1 is rewritten from a greenfield build into a brownfield adoption.** A working mock of the **Judicial Office eLinks People API v5** already exists, written from vendor documentation (`Swagger UI.pdf`, `apiresponses.docx`) and serving the **real production reference-data exports** of 2026-06-01. Epic 0M.1's original premise — scaffold a Spring Boot service and serve fixtures shaped by *guessing* the contract from `data-tables.md` — is obsolete. The epic now covers **adoption + hardening + the two capabilities the implementation lacks**: Story 0M.1.1 (rename to `ctam-joh-mock`, container, Helm, probes, structured logs into App Insights, the production-refusal guards — **currently absent** — CI with audit + SBOM, and a data-governance check on the production exports) and Story 0M.1.2 (machine-readable OpenAPI, a guaranteed **ET cohort**, fixture-identity alignment across all three mocks, and **fault injection** — also currently absent). **Stack: Node 22 LTS + Express 4, accepted as the single documented exception to AR2–AR17 (new AR55.1)**, scoped to language and build tooling only — operability, observability, security and contract discipline are all still required, and the exception is explicitly not precedent. **`depends_on` drops `epic-0.1`**: the mock's shape now derives from the real contract rather than from CTAM's `jo_*` schema, which inverts the information flow (Epic 0.1's schema should be validated *against* this contract). **Four load-bearing assumptions are falsified by this evidence** — F1 the API is a **change feed** with explicit leaver/deleted signals, not a full-refresh dump (contradicts **AR46**); F2 `REF_Jurisdiction` is **flat**, hierarchy lives on `locations.parent_id` / `ticket_categories.parent_category_id` (contradicts **D8**/G8.1); F3 **ET is a location, not a jurisdiction**, so **FR57's `(jurisdiction, region)` activation key cannot express ET** (contradicts G8.1's wave-1 condition); F4 the contract exposes `per_id`/`personal_code` and has **no `personnel_number`** (contradicts **D9**). These are **recorded in Epic 0M.1's *Findings* section and gaps.md G8.1, and deliberately not actioned here** — the AR46/D8/D9/FR57 corrections are a separate architect-led change. **G8.5 is partly evidenced** (ET legal role titles confirmed real; the employer/employee-side lay-member panels appear nowhere — still provisional). **New gap G8.7** covers data-owner governance of the production exports. **G8.1 stays open** — no live connection, cadence/SLA unconfirmed, MRD half untouched. **No FR or NFR changed; no code impact to any other repo.** See [`../sprint-change-proposal-2026-08-18d.md`](../sprint-change-proposal-2026-08-18d.md). |
...
- **Failure handling** — a failed sync leaves the previous good state in place (each ingestion is transactional per entity set); failures surface via structured logs + `ctam_sync_status` for ops triage. Reference data is at most one cycle stale, never partially written. Every one of these failure paths is **reproducible on demand** via the fault-injection / variant modes of `ctam-joh-mock` and `ctam-mrd-mock` (AR55, AR56) — see *Phasing of Upstream Integrations* below.
- **Non-production upstream** — both ingestion paths run against contract-shaped stand-ins in dev/CI/integration (`ctam-joh-mock`, `ctam-mrd-mock`) because neither real contract is confirmed (G8.1). The stand-ins are deployables reached over a real HTTP / blob hop, not in-process stubs, so the integration is genuinely exercised; switching to the real upstream is configuration only.
...
#### Phasing of Upstream Integrations: Mock-First, Real Upstream Later

The same mock-first reasoning that governs authentication (below) governs the **upstream data plane**. Neither the JOH eLinks contract nor the MRD feed arrangement is confirmed (gaps.md **G8.1**, a wave-1 blocker), so each gets a deployable stand-in built **before** the ingestion epic that consumes it. The two stand-ins are delivered as their own **pre-Phase-0 tier, Phase 0-Mock** (epics `0M.1`/`0M.2` — see `epics/phase-0-mock/index.md`), so that the stand-in work is visibly separate from the foundations it enables and no reader infers its sequencing from an epic number.

**`ctam-joh-mock` (Epic 0M.1 deliverable, Phase 0-Mock, AR55 revised — adopted brownfield):**

- **The contract is documented, not guessed.** A working implementation of the **eLinks People API v5** already exists and was adopted rather than rebuilt (decision #19). It was written from vendor documentation (`Swagger UI.pdf`, `apiresponses.docx`) and serves the **real production reference-data exports** dated 2026-06-01 — Locations 1,999 rows, BaseLocations 1,461, AppointmentTitles 193, JudiciaryRoles 163, Tickets 158 (organisational reference data only; no personal data — gaps.md G8.7). People are synthetic, generated deterministically under a fixed seed.
- **Endpoint surface:** `reference_data/:attribute_name[/:reference_id]`, `people/:id`, the `people?updated_since=` **change feed**, and the `leavers?left_since=` / `deleted?deleted_since=` feeds — bearer-authenticated, on both bare and `/elinks` base paths, with the documented pagination and error envelopes.
- **The contract is a change feed, not a full-refresh dump** — and departure/deletion arrive as explicit signals rather than being inferred from absence. This **contradicts AR46** and is recorded as finding **F1** on Epic 0M.1 and in gaps.md G8.1; the correction is owned by a follow-up SCP.
- **ET is expressed as a location, not a jurisdiction** (1036 ET E&W under `jurisdiction_id` 34 Tribunals, plus 1035 EAT / 1037 ET Scotland), backed by ET tickets and 10 ET appointment titles; `REF_Jurisdiction` is flat with no `parent_id`. Findings **F2/F3** — again recorded, not yet actioned. Fixtures must guarantee a documented ET cohort against that predicate.
- **Fault-injection modes** — malformed payload, per-record non-conformance, unreachable / 5xx, slow response, plus change-feed-specific faults (a feed shifting between page requests; an inconsistent pagination envelope) — making Story 0.2.3's failure-path criteria executable. Not present in the adopted implementation; added by Story 0M.1.2.
- Publishes a machine-readable **OpenAPI 3.x** spec as **CTAM's reading of the eLinks v5 contract from vendor documentation — not a contract agreed with Judicial Office** — with each field marked *evidenced* or *inferred*.
- **Node 22 LTS + Express 4 — the single documented exception to AR2–AR17** (AR55.1), scoped to language and build tooling only. Container, Helm chart, probes, structured JSON logs with correlation IDs, App Insights ingestion, CI gates, SBOM and published contract are all still required.
- **Production safeguard:** refuses to start when the environment indicates production; excluded from `deploy-production.yml`; CI lint blocks production manifests naming it as an eLinks endpoint.

**`ctam-mrd-mock` (Epic 0M.2 deliverable, Phase 0-Mock, AR56):**

- Generates an MRD-shaped `.xlsx` from a version-tagged fixture set whose Specialisations reference the same personnel numbers as the JOH fixtures, so the ingestion's **referential** cleansing checks resolve against JOH data that exists.
- Publishes into a **configurable** blob container — local **Azurite** in CI — on demand and on the real weekly `@Scheduled` cadence, in **conformant / per-row-non-conformant / structurally-invalid / cross-sheet-lookup-failure / byte-identical-re-drop** variants, each driving a specific Story 0.3.1 acceptance criterion.
- **Does not provision the drop container** — that stays in `ctam-reference-data`'s own `terraform/` (AR47, AR53), because the consumer owns the resource the real MRD team drops into. Being container-target-agnostic also keeps the dependency edges acyclic: the mock never depends on the ingestion epic it feeds.
- Publishes a **column dictionary** as CTAM's provisional written expectation of the workbook.
- **Production safeguard:** same three guards as above.

**Neither mock closes G8.1.** They encode CTAM's assumptions, not evidence about the real upstreams. What they buy: the ingestion code paths and *all* their failure paths are built, demoed and CI-gated before upstream access exists; the real contract arrives as a **diff against an executable expectation** rather than an open discovery; and the cutover is configuration, not code. G8.1 closes only when each ingestion has run end-to-end against real upstream data and ET jurisdiction coverage is confirmed.

**Environments:** local dev, CI, and integration run against the mocks. Production runs against real eLinks and the real MRD feed only. Like `ctam-mock-auth`, the mocks are **not retired at cutover** — they remain the non-production upstream for the life of the programme; what changes on G8.1 closure is that their fixtures and published contracts are reconciled against reality and re-tagged.
...
- Non-production **upstream** data comes from `ctam-joh-mock` and `ctam-mrd-mock` (AR55, AR56). Real eLinks and the real MRD feed are production-only until G8.1 closes; the cutover is configuration, not code.
...
3. Ship `ctam-mock-auth` (Spring Authorization Server; refuses to start with `production` profile; supports `authorization_code` and `client_credentials`), and the two **upstream source stand-ins** `ctam-joh-mock` (AR55) and `ctam-mrd-mock` (AR56) — each ahead of the ingestion epic that consumes it, and each refusing the `production` profile the same way.
