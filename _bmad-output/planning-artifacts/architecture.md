...
amendmentSource: '_bmad-output/planning-artifacts/sprint-change-proposal-log.md#sprint-change-proposal-2026-06-10'
inputDocuments:
...
- **Manual UAT** (FR60 / NFR41) — jurisdiction-incumbent-experienced users compare CTAM Pathfinder vs the incumbent per service per wave (`[ET-INCUMBENT-TBD]`-experienced users for ET wave 1; ListAssist-experienced users for SSCS wave 2; APEX-experienced users for Courts waves 3+)[^d13]; sign-off is the wave-cutover gate. No automated incumbent-comparison in CI.
...
| 12 | Integrations-first Phase 0 sequencing + integration-component hosting *(SCP 2026-06-17)* | **`ctam-reference-data` is the first service scaffolded.** The JOH eLinks + MRD ingestion is carved out as the **first Phase 0 epic**, decoupled from the auth/UI slice it was previously bundled into. Ingestion stays **in-process** inside `ctam-reference-data` (reaffirms #9/#10); **a separate `ctam-integrations` repo is declined** — tier-(a) tables are owned solely by `ctam_reference_data` (AR49), so a new deployable would break the single-writer invariant and reopen the service-auth gap (G7) for no resilience gain. Per the **AR53 first-consumer rule**, the **shared Azure estate Terraform relocates from `ctam-authorisation` to `ctam-reference-data`**. *(The shared-estate relocation part of this decision is **superseded by #13**.)* The jurisdiction-filtered Reference Data **read API remains downstream of auth** (needs `JWTFilter` for token validation + `authz/check` for the jurisdiction it filters on, D8), so it lands immediately *after* the auth epic, not in the first phase. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-06-17`](sprint-change-proposal-log.md#sprint-change-proposal-2026-06-17). |
| 13 | Shared infrastructure moves to a dedicated repo — CNP alignment *(SCP 2026-07-06)* | **Supersedes the AR53 "colocated first-consumer" rule from #12.** Per the HMCTS Cloud Native Platform standard that product-level shared infrastructure lives in a `{product}-shared-infrastructure` repo, the **shared Azure estate** (AKS, PostgreSQL, ACR, APIM, App Insights, Key Vault) moves **out of `ctam-reference-data`** into a new dedicated repo, **`ctam-shared-infrastructure`** (16 repos total). It is provisioned and **independently verified** in the new **Epic 0.0** — 5 stories, each a Terraform layer with a deploy-time acceptance test — ahead of any service. Service repos keep only their **own** per-repo Terraform. **AR53 revised.** Story 0.1.1 reduces to scaffold-and-deploy-onto-estate. Phase 0: 5 → 6 epics, 14 → 19 stories. **No code impact** (implementation not started). See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-07-06`](sprint-change-proposal-log.md#sprint-change-proposal-2026-07-06). |
| 14 | Interim substitute for the unconfirmed JOH eLinks contract *(SCP 2026-08-24)* | **A deployed, schema-faithful mock — `ctam-jomockapi` (17 repos total)** — replaces "CI-only WireMock stub" as Phase 0's dev/staging demonstrability story, without closing G8.1. Onboarded as a new **Epic 0.7** *(renumbered to **Epic 0.2** by #16)*, in the same non-production-only category as `ctam-mock-auth` (repository-strategy.md). Story 0.1.3 gains one AC block pointing its dev/staging sync target at the deployed mock; the existing CI WireMock stub is retained unchanged. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-24`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-24). |
| 15 | Epic 0.1 retitled and narrowed to JOH-only; MRD split into Epic 0.8 *(SCP 2026-08-24b)* | *(Epic 0.1 throughout this row is the JOH ETL process, as numbered at the time; renumbered to **Epic 0.3** by #16 — Epic 0.8/MRD is unaffected.)* Phase 0's focus for Epic 0.1 is **JOH data only**. Epic 0.1 is retitled **"JOH Reference-Data ETL Process"** — "ETL" here labels the **ongoing** in-process nightly `@Scheduled` sync (decision #9, unchanged technically), deliberately reusing a term retired in 2026-06-11 for a *different*, one-shot legacy-migration tool (gaps.md G4.6) — the two are explicitly distinguished, not conflated. **MRD ingestion (formerly Story 0.1.4) is split out into a new Epic 0.8**, same repo (`ctam-reference-data`), same `ctam_sync_status` run-log, depends on Epic 0.1 (needs its scaffold + tier-(a) write-protection pattern). Story renumbering: Epic 0.1 keeps Stories 0.1.1–0.1.3 only; the old Story 0.1.4 becomes Story 0.8.1, content unchanged. Total Phase 0 story count unaffected (24). See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-24b`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-24b). |
| 16 | Minimal 5-epic renumber: schema-design/mock-API/ETL swapped to 0.1/0.2/0.3, displacing user-auth/read-API to 0.7/0.9 *(SCP 2026-08-25b)* | **Relabeling only** — every epic's real technical dependencies are unchanged, rewired to the new ids. The JOH domain-schema epic (0.9, itself just relocated from Phase 1 by #16's predecessor SCP 2026-08-25) → **0.1**; the JOH eLinks mock API (0.7) → **0.2**; the JOH reference-data ETL process (0.1) → **0.3**, displacing User authenticates (0.2) → **0.7** and Reference data read-only API (0.3) → **0.9** into the two slots vacated by the incoming epics. A closed permutation — Epics 0.0, 0.4, 0.5, 0.6, 0.8 untouched, no gaps. Every `depends_on` array, cross-reference, and Story id across the 5 renumbered epics (and every other epic that referenced them) was rewired to match. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25b`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25b). |
| 17 | Reference Data read API split by upstream source; JOH-data epic placed at 0.4, displacing bootstrap to 0.9 *(SCP 2026-08-25c)* | Epic 0.9's read API (per #16) never actually covered `mrd_*` data — only tier-(b) vocab and tier-(a) JOH endpoints were ever specified. Split by upstream source, mirroring the Epic 0.3 (JOH ingestion) / Epic 0.8 (MRD ingestion) write-side split: **JOH data read API** (unchanged content, retitled) placed at **Epic 0.4** per an explicit follow-up request, displacing "User populations bootstrapped" to the vacated **Epic 0.9** (same minimal-swap resolution as #16). **New Epic 0.10** adds the first `mrd_*` read endpoint (`GET /v1/reference-data/specialisms`) — genuinely new scope, not a relabel — same API-as-Product conventions, published in the same `api-ctam-reference-data` OpenAPI artefact (one service, one contract). See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25c`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25c). |
| 18 | Phase 0 scope narrowed to epics 0.0–0.4; auth, notification, MRD, and bootstrap removed for now *(SCP 2026-08-25d)* | Phase 0 is narrowed to **just the JOH data pipeline**: platform estate, JOH domain schema, JOH eLinks mock API, JOH ETL process, JOH data read API. Removed from the active plan (not deleted from history — each retains its git history and is re-plannable): Notification (was 0.5), User authenticates (was 0.7), MRD ingestion (was 0.8), User populations bootstrapped (was 0.9), MRD data read API (was 0.10). **Two real dependency edges resolved:** the context bus + shared `ctam_configuration_values` baseline (formerly Epic 0.6) is **folded into Epic 0.0** as Stories 0.0.6–0.0.7 (Story 0.0.6, "publish the context bus," was already `done` — carried forward, not reset); Epic 0.4's read API — which needed `JWTFilter` + jurisdiction filtering from the removed auth epic — is **simplified to an open, unauthenticated API** for this scope, re-addable additively when auth returns. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25d`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25d). |
| 19 | Epic 0.2 (JOH eLinks mock API) narrowed to local-only — no shared-estate deployment *(SCP 2026-08-25f)* | `ctam-jomockapi` runs **locally via Docker Compose** only, alongside a locally-run `ctam-reference-data` — the shared AKS/ACR/Key Vault/APIM deployment Story 0.2.2 previously specified is replaced with a Docker Compose equivalent, and Story 0.2.3's "dev/staging demonstrability" claim narrows to "local demonstrability." **`depends_on` drops from `[epic-0.0]` to `[]`** — the only reason Epic 0.2 depended on Epic 0.0 was that deployment, which no longer exists in this epic's scope. NFR16 (Key Vault), NFR31 (Azure UK South) and NFR40 (per-service deployable) are no longer exercised by this epic; NFR24's "exercised end-to-end pre-contract" claim narrows to local-only, not a shared-environment demonstration. gaps.md G8.1 and non-functional-requirements-coverage.md's NFR24 entry are reworded accordingly — G8.1 itself stays open regardless. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25f`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25f). |
| 20 | Epic 0.2's `ctam-jomockapi` rebuilt as Java/Spring Boot instead of Node.js/Express *(SCP 2026-08-25g)* | The explicitly-scoped Node.js/Express stack deviation recorded in assumptions.md A38 is **retired**. `ctam-jomockapi` is now scaffolded from the same HMCTS Crime SpringBoot template + `ctam-scaffold.sh` conventions as every other CTAM Pathfinder service (AR2–AR9) — Story 0.2.1 is rewritten from "onboard the existing Node codebase" to "scaffold from the HMCTS starter and **faithfully port** the existing Node/Express reference implementation's documented contract (endpoints, fixed-seed data, auth behaviour) to Java/Spring Boot." This is a stack port, not a redesign — no behavioural contract change. Unlike every domain service, no Liquibase/Testcontainers PostgreSQL/MapStruct are added (the mock stays stateless, no database) and no Helm chart/per-service Terraform are added (it still never deploys anywhere but locally, per #19). `repository-strategy.md` and assumptions.md A38 are revised in place (not superseded — these are living reference docs, not dated records). See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25g`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25g). |
| 21 | Five cross-epic inconsistencies found on review, fixed *(SCP 2026-08-25h)* | A full read-through of all 5 Phase 0 epics surfaced five inconsistencies, all fixed: (1) **port collision** — Story 0.1.1 (`ctam-joh`) and Story 0.3.1 (`ctam-reference-data`) both hardcoded port 8082 per AR3, which itself gave no per-service override; AR3 revised to assign each service its own port (`ctam-joh` moves to 8083), consistent with `ctam-jomockapi`'s existing 8090. (2) **FR8 double-attributed** — Epic 0.3's "FRs covered" line claimed FR8 "first lands here," contradicting `fr-coverage-map.md` and Epic 0.0 Story 0.0.7, which actually delivers it; Epic 0.3 now says it only consumes/verifies the baseline. (3) **wrong story citation** — Epic 0.4's Story 0.4.1 cited Story 0.3.3 (the sync) as its tier-(a)-tables prerequisite; it needs Story 0.3.2 (the tables themselves), not populated data. (4) **`ctam_joh_identities` never explicitly created** — Story 0.3.2's changeset AC enumerated the 15 `jo_*` tables + `ctam_sync_status` but never listed `ctam_joh_identities`, despite three other places (Story 0.3.3, Epic 0.1 Story 0.1.2, Epic 0.3's own prose) depending on it existing; added explicitly to Story 0.3.2. (5) **local-mock vs. AKS-deployed reference-data tension** — Story 0.3.3's mock-integration AC (against the locally-run `ctam-jomockapi`, per #19) didn't say which run-mode of `ctam-reference-data` it applied to, while Story 0.3.1 also describes an AKS deployment with no path to a developer's Docker Compose network; the AC now states explicitly that it exercises `ctam-reference-data` run locally too (`./gradlew bootRun`), not the AKS-deployed instance. A sixth observation — whether Epic 0.0's full Terraform-provisioned estate is still the right scope given Epic 0.2's local-first move — was raised as an open policy question, not fixed, since it needs a product decision, not a documentation correction. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25h`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25h). |
| 22 | Two more cross-epic inconsistencies found on a second review pass, fixed *(SCP 2026-08-25i)* | A second full review of all 5 Phase 0 epics — this time cross-checking against `architecture/data-tables.md` and `requirements-inventory.md`'s AR list directly, not just epic-to-epic — surfaced two more: (1) **`ctam_joh_identities` grant breadth understated** — Story 0.3.2's grants AC only named `ctam_joh` and "placeholder roles" for SELECT, but `data-tables.md` documents `ctam_joh_identities` as SELECT-granted to **every** domain service (it's the JOH spine every domain table keys off of, not a `ctam_joh`-specific concern); the AC now says so explicitly, and the fitness-function AC verifies grant breadth too. (2) **AR52 mis-cited for WireMock/stub testing** — Epic 0.2 (Story 0.2.3, twice) and Epic 0.3 (Story 0.3.3) all cited "AR52" for the CI-only WireMock/stub eLinks API convention, but AR52 is actually about CTAM-internal auth/bootstrap records — no AR anywhere actually documented the WireMock convention. A new **AR54** was added to `requirements-inventory.md` describing it, and all three stale citations (plus one incomplete References line) were corrected to point at it. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25i`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25i). |
| 23 | Epic 0.1 expanded to design `ctam-joh`'s schema with the same rigor as the JOH eLinks integration schema *(SCP 2026-08-25j)* | Two Confluence pages surfaced the JOH eLinks integration schema's own design documentation (`3.2.1.1 CTAM JO Schema`, `3.2.1.2 CTAM Schema and Source Mapping` — distilled to `_bmad-output/source-docs/joh-schema-confluence/` *(relocated out of the repo by #26)*), which describes the **tier-(a) `jo_*` schema owned by `ctam-reference-data` / Epic 0.3**, not Epic 0.1's tier-(b) `ctam-joh` overlay tables — a different, non-overlapping table set in a different repo. Rather than duplicating `jo_*` content into Epic 0.1, this decision adopts the JOH eLinks schema's **documentation pattern** — a written design-decisions doc, an ER diagram (D2+ELK, `sql_table` shapes), a full column-level spec, and a data-warnings/open-questions companion — for Epic 0.1's own 5 tables. Epic 0.1 grows from 2 stories to 4: 0.1.1 (scaffold, unchanged) → 0.1.2 (new: design decisions) → 0.1.3 (rewritten: ER diagram + column-level DDL) → 0.1.4 (new: data warnings/open questions). **Not actioned by this SCP, flagged as a follow-up:** Epic 0.3 Story 0.3.2 and `data-tables.md`'s `jo_*` section still hold table-name-only definitions with no column-level detail — the new source pages are a candidate to close part of `gaps.md` G8.1 (the JOH eLinks contract was previously unconfirmed) via an equivalent pass against Epic 0.3, not done here. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25j`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25j). |
| 24 | Epic 0.2's mock-API contract corrected against the real E-links API v5.0 Swagger doc *(SCP 2026-08-25k)* | The real `E-links API v5.0` Swagger doc and June 2026 production reference-data exports (distilled to `_bmad-output/source-docs/joh-elinks-api/` *(relocated out of the repo by #26)*) surfaced two endpoints Story 0.2.1's "faithful reimplementation" AC omitted — `GET /api/v5/people/{id}` (single-person lookup) and `GET /api/v5/reference_data/{attribute_name}/{reference_id}` (single reference-data item lookup) — plus unspecified query-parameter contract detail: `updated_since`/`left_since`/`deleted_since` are **required** (400 on omission), `per_page`/`page` are optional and paginated per the documented `PaginationResponse` shape, and each of the 11 reference-data vocabularies is also reachable via a documented deprecated singular alias. Story 0.2.1's endpoint AC is corrected to name all of this explicitly — no behavioural redesign, the mock's contract scope was simply under-specified. **Explicitly decided against:** mounting the mock under the real API's `/elinks` gateway-routing path segment (visible in the Swagger doc's `Servers` value) — judged to be production gateway routing, not part of the application contract, and irrelevant to a local mock with no gateway in front of it; the mock keeps its existing bare `/api/v5/...` routes. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25k`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25k). |
| 25 | Epic 0.2's stories gain "Implementation touchpoints" guidance, not a literal `## Tasks / Subtasks` section *(SCP 2026-08-25l)* | A request to add API/service/repository/UI/test breakdown to Epic 0.2's stories, under the heading `## Tasks / Subtasks`, would have collided with `delivery-operating-model.md`'s documented contract: that exact heading is BMad-reserved, generated by `bmad-create-story` into the story-packet at dispatch time and read/written by exact heading by `bmad-dev-story` — the same file records a prior incident (SCP 2026-08-19b) caused by exactly this kind of heading collision. **Resolution:** the requested content is added under a differently-named subsection, **"Implementation touchpoints,"** explicitly framed as dispatch guidance for `bmad-create-story`, not the BMad-generated artifact itself. Applied to all three of Epic 0.2's stories; not retrofitted to any other epic by this decision. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25l`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25l). |
| 26 | `_bmad-output/source-docs/` relocated out of the repo entirely *(SCP 2026-08-25m)* | The two source-doc archives created by #23 (`joh-schema-confluence/` — two distilled Confluence PDFs) and #24 (`joh-elinks-api/` — the real Swagger doc plus **real HMCTS production reference-data exports**, `eLinks_Pivotl_Production_all-data_2026-06-01_*`) are moved out of the repo entirely, to a local, non-versioned path on the requester's machine (`/Users/shivakumar/MOJ/docs/source-docs/`), at explicit user request. Epic 0.1 and Epic 0.2's References/design-rigor text, which cited these archived paths, are corrected to no longer claim an in-repo copy exists — the underlying citations (the Confluence page titles, the real Swagger doc, the reference-data exports as a source) remain valid; only the claim of an in-repo archived copy is removed. **Not done:** a git-history rewrite — the files remain recoverable from the commits that added them (`a4bd168`, `3be19ef`) unless a separate, explicit history-rewrite request is made. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25m`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-25m). |
| 27 | All 29 Sprint Change Proposals consolidated into one file *(SCP 2026-08-26)* | The 29 separate `sprint-change-proposal-{date}.md` files (2026-05-15 → 2026-08-25m) are merged, in chronological order, into a single **`sprint-change-proposal-log.md`** — each original file becomes one section, content preserved verbatim (only frontmatter folded into the log's intro, and heading levels demoted to nest under the log's own title). The 29 originals are deleted. **111 cross-references** across `architecture.md`, `architecture/changelog.md`, `prd.md`, `prd-validation-report-2026-06-10.md`, `implementation-readiness-report-2026-05-15-rev2.md`, `epics/phase-0/index.md`, and `business-case.md` (plus 15 SCP-to-SCP references now internal to the log) are rewritten from the old per-date filenames to `sprint-change-proposal-log.md#sprint-change-proposal-{date}` anchors. `scripts/python/build_html.py`'s NAV collapses ~18 individual SCP entries into one. This retires the *one-file-per-proposal* filing convention (not the "don't rewrite historical content" rule in `CLAUDE.md`, which this change complies with — every entry's substance is untouched). Future `bmad-correct-course` runs still default to a new dated file (that default lives in the installed skill, outside this repo); folding new runs into this log going forward is a manual choice, not automatic. See [`sprint-change-proposal-log.md#sprint-change-proposal-2026-08-26`](sprint-change-proposal-log.md#sprint-change-proposal-2026-08-26). |
...
| **HMCTS judicial region** | The per-region boundary *within* a jurisdiction. Per D8 (reframed 2026-06-10), the rollout boundary is **jurisdiction first, then per-region within jurisdiction** — wave 1 is the whole **Employment Tribunals** jurisdiction[^d13]; wave 2 is the whole SSCS jurisdiction; waves 3+ are Courts jurisdictions with per-region granularity (Northern, Western, etc.). | Application tier: per-user activation flag in `ctam_auth_user_activation_flags` carrying the (jurisdiction, region) tuple (FR57). No infrastructure isolation per jurisdiction or HMCTS region. |
