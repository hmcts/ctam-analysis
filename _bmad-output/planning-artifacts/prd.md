...
CTAM Pathfinder (**CTAM** = **Court and Tribunals Availability Management**) is HMCTS's greenfield platform for judicial scheduling across **Tribunals and Courts**. 11 services, modern UI, Azure-deployed. It replaces **ListAssist** (the SSCS judicial-scheduling tool) in Tribunals (SSCS first, wave 1) and the as-is **JI application on unsupported Oracle APEX (OPT)** in Courts (waves 2+). Built in isolation from the incumbents; cutover is jurisdiction by jurisdiction, then per region within Courts.
...
- **ListAssist** — the SSCS judicial-scheduling tool through which SSCS Tribunals schedule judicial sittings today. Replaced by CTAM Pathfinder in **wave 1** (Phase 9). (GAPS, the SSCS case-management system, is **retained**, not decommissioned — analogous to the external case-management systems that consume CTAM's APIs per D12.)
...
*SSCS Tribunals jurisdiction (wave 1) — applicable roles will be enumerated against the ListAssist as-is analysis pack (parallel to the JI pack under `docs/architecture/asis/`). Working set:*
...
*Operational platform support (OPT Support in the legacy JI system; equivalent in ListAssist) is handled by external HMCTS roles and is not a CTAM Pathfinder user role.*

**Problems being solved:**

1. **Both incumbents have replacement drivers.** In Courts, APEX is unsupported with a fixed end-of-life. In Tribunals (SSCS), judicial scheduling runs on ListAssist, which CTAM Pathfinder replaces in wave 1. (GAPS, the SSCS case-management system, is retained, not decommissioned.)
...
**Success:** SSCS jurisdiction migrated to CTAM Pathfinder in wave 1 with ListAssist replaced for SSCS scheduling (GAPS case management retained); every HMCTS Courts judicial region migrated in waves 2+ with APEX/JI retired; downstream consumers integrating via API; future HMCTS programmes building on CTAM Pathfinder's APIs.

### Key characteristics

1. **Greenfield, not strangler.** Neither ListAssist nor APEX supports strangler decomposition. CTAM Pathfinder is built end-to-end before any user moves; the legacy system in each jurisdiction runs unchanged for non-migrated users during phased rollout. No dual-write, no event bus, no synchronisation layer.
...
3. **The jurisdiction's incumbent system is the behavioural reference, verified by manual UAT[^d5][^d11].** UAT is performed by users with hands-on experience of *that jurisdiction's* incumbent system — ListAssist-experienced users (RTJ, Tribunal Judges, Tribunal Members, Caseworkers, Finance, MI) for wave 1; APEX-experienced users (RSU, Court, Judge, Judges' Clerks, Finance, MI) for waves 2+. They compare CTAM Pathfinder behaviour against the incumbent side-by-side. No automated comparison harness; legacy systems are not co-managed[^d6].
...
5. **Per-jurisdiction, per-region phased cutover[^d8][^d11].** Wave 1 is the SSCS Tribunals jurisdiction. Waves 2+ are Courts regions, one HMCTS judicial region per wave, with all applicable in-region roles together. Migrated users do not use the legacy system; non-migrated users do not use CTAM Pathfinder. No contention or synchronisation.

6. **One decomposition serves both jurisdictions[^d11][^d12].** The same 11 services cover SSCS: tribunal-member sub-types (Medical, Disability-Qualified, Disability (Other)) are additional JOH types handled by `ctam-joh`, `ctam-booking` and `ctam-sitting`. Panel composition for specific cases and hearing-type tracking live in **external case-management systems**[^d12], not in CTAM.

**Why now:** SSCS is the chosen wave-1 jurisdiction[^d11], where ListAssist (the SSCS judicial-scheduling tool) is replaced (GAPS case management is retained). APEX is unsupported with its own end-of-life and is replaced in waves 2+. HMCTS programmes need integration patterns the export-only legacy cannot deliver. Each month on the legacy systems delays API integration for the wider ecosystem.
...
Each role can complete its legacy workflow on CTAM Pathfinder without re-training, and faster or no slower than the **cohort's incumbent system** (ListAssist for the SSCS jurisdiction in wave 1; APEX for Courts jurisdictions in waves 2+):
...
- **All roles**: WCAG 2.2 AA UI; performance baseline meets the cohort's incumbent page-level NFRs (per the Courts/APEX baseline per NFR1–NFR5; SSCS wave 1 verified against ListAssist-equivalent operations per the readiness assessment).

### Business Success

- **Legacy retirement** — ListAssist (SSCS judicial scheduling) replaced for SSCS in wave 1 (GAPS case management retained, not decommissioned); Oracle APEX (OPT) decommissioned as every Courts judicial region migrates in waves 2+.
...
- **Behavioural parity**[^d5] — manual UAT script per domain service, walked by **jurisdiction-incumbent-experienced users**: ListAssist-experienced users (RTJ, Tribunal Judges, Tribunal Members, Caseworkers, Finance, MI) for wave 1; APEX-experienced users (RSU, Court, Judge, Judges' Clerks, Finance, MI) for waves 2+. Sign-off is the wave gate.
...
- **Performance NFRs** met or exceeded (≤ 5 s dashboard refresh; ≤ 10 s list/filter; ≤ 15 s batch/annual; ≤ 30 s reports/Forward Look). Page-level NFRs are derived from the APEX baseline; SSCS wave-1 cutover verifies these against ListAssist-equivalent operations as part of the SSCS-cohort readiness assessment.
...
| Behavioural parity per domain service | 100% of manual UAT scripts (run by jurisdiction-incumbent-experienced users — ListAssist-experienced for wave 1, APEX-experienced for waves 2+ — comparing CTAM Pathfinder vs the incumbent) signed off before that wave's cutover | D5 reframed + FR60 |
...
| Page-level performance | All page-level NFRs from `functional-modules.md` met or exceeded; SSCS wave-1 cutover verifies against ListAssist-equivalent operations | functional-modules.md cross-cutting NFRs |
...
- **Behavioural parity with the jurisdiction's incumbent** verified through **manual UAT performed by jurisdiction-incumbent-experienced users** (D5 reframed; FR60) — ListAssist-experienced users for wave 1, APEX-experienced users for waves 2+. There is no automated incumbent-comparison test harness in the MVP.
...
- Bi-temporal history
- Historical-data access at cutover (D3 superseded by D11 — no migration; historical scheduling data stays in the cohort's incumbent system: ListAssist for SSCS, APEX for Courts; access policy per Risk #2 is a separate decision)
...
- **Historical-data access policy** at cutover — read-only incumbent-system bridge (ListAssist for SSCS / APEX for Courts), or one-shot export, or policy-of-no-access (Risk #2).
...
2. Asha opens the absence. CTAM displays Dr. M's profile (sourced from `jo_people` per FR11), Medical-Member appointment type (sourced from `jo_appointments`), Specialisations (sourced from MRD), tickets/authorisations (tier-(a) `jo_tickets` + CTAM-overlay per FR15), and the dates of absence.
3. Asha approves the absence. CTAM auto-creates a vacancy (FR23) pre-populated with JOH type (Medical Member), jurisdiction (SSCS), date range.
4. Asha advertises the vacancy out-of-system (the standard SSCS process — phones available Medical Members; CTAM's availability view per FR18 + FR27 helps her shortlist).
5. Dr. P, another Medical Member with the right Specialisations, confirms availability.
6. Asha creates a booking in CTAM (FR29) — JOH = Dr. P, jurisdiction = SSCS, tribunal venue, date range, session type. The vacancy is marked filled (FR30).
7. CTAM emails Dr. P a booking acknowledgement (FR32).
8. After the hearings happen (managed externally[^d12]), Asha confirms the sitting (FR37) in CTAM, recording whether Dr. P actually sat.
...
| **External case-management systems** (SSCS case management; Courts Listing systems)[^d12] | Outbound (APIs) | From Phase 9 (wave 1 onward) | REST APIs from CTAM | CTAM exposes JOH availability + booking data for external case-management consumption. No reverse-write into CTAM (CTAM is the system of record for JOH scheduling). |
| **Future programmes** (Actuals, Scheduling & Listing reforms) | Outbound (APIs) | Post-MVP | REST APIs from CTAM | Vision-level; specific contract design happens when programme demand crystallises. (Tribunals removed from "future programmes" — SSCS is wave 1[^d11].) |
...
- **NFR32 — Retention**[^d11][^d3]: Data retention per HMCTS retention schedules. **No data is migrated** — historical transactional data remains in the cohort's incumbent system (ListAssist for SSCS, APEX for Courts); CTAM Pathfinder retains only data created in CTAM Pathfinder from cutover onward.
...
- **NFR36 — Per-wave rollback**[^d11]: Each rollout wave (Phase 9, 10, …) has a documented rollback path returning the affected wave's users to the jurisdiction's incumbent system within one operational cycle if the wave's gate is breached post-cutover — to **ListAssist** for wave 1 (SSCS), to **APEX** for waves 2+ (Courts regions).
...
| **D5** *(revised 2026-05-06; reframed 2026-06-10 per D11)* | **The jurisdiction's incumbent system is the behavioural reference, verified by manual UAT performed by users experienced in that incumbent system.** Neither incumbent (ListAssist, APEX) is a migration host. For wave 1 (SSCS): ListAssist-experienced users — RTJ, Tribunal Judges, Tribunal Members, Caseworkers, Finance, MI. For waves 2+ (Courts): APEX-experienced users — RSU, Court, Judge, Clerks, Finance, MI. | Per-service manual UAT scripts are walked through by the in-jurisdiction, in-wave applicable users comparing CTAM Pathfinder vs the jurisdiction's incumbent side-by-side; sign-off per role per wave is the wave gate. No automated incumbent-comparison harness; no co-management of either incumbent (D6). |
...
| **GAPS** | The **SSCS case-management system** (legacy), **retained** — CTAM Pathfinder does **not** replace GAPS. In wave 1 CTAM replaces the SSCS judicial-scheduling tool (**ListAssist**), not GAPS. GAPS is the SSCS analogue of the external case-management systems that consume CTAM's APIs (per D12): it stays in place and consumes CTAM's APIs[^d11][^d12]. |
...
| **JI** | Judicial Itineraries — the legacy Oracle APEX system serving the **Courts** jurisdiction. Replaced by CTAM Pathfinder in waves 2+[^d11]. |
...
| **ListAssist** | The SSCS judicial-scheduling tool used in the SSCS Tribunals jurisdiction today; the SSCS scheduling incumbent and the wave-1 UAT parity reference. Replaced by CTAM Pathfinder for SSCS in wave 1[^d11]. (GAPS, the SSCS case-management system, is retained.) |
...
| **CTAM Pathfinder** | The API-driven greenfield platform for HMCTS judicial scheduling this PRD describes. Replaces ListAssist for SSCS in wave 1 and JI/APEX for Courts in waves 2+. |
...
| **SSCS** | Social Security and Child Support — the HMCTS Tribunals jurisdiction handling appeals on PIP, ESA, Universal Credit, DLA and related benefits decisions. The wave-1 jurisdiction for CTAM Pathfinder[^d11]. |
| **SSO** | Single Sign-On |
