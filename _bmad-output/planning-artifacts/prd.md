---
type: 'PRD'
title: 'Product Requirements Document - ram-analysis'
description: 'Author: Ramnish'
resource: 'prd.html'
tags: [ram-pathfinder, prd, employment-tribunals]
...
- **ListAssist** — the SSCS judicial-scheduling tool through which SSCS Tribunals schedule judicial sittings today. Replaced by RAM Pathfinder in **wave 2**[^d13]. (GAPS, the SSCS case-management system, is **retained**, not decommissioned — analogous to the external case-management systems that consume RAM's APIs per D12.)
...
*Operational platform support (OPT Support in the legacy JI system; equivalents in ListAssist and the ET incumbent) is handled by external HMCTS roles and is not a RAM Pathfinder user role.*

**Problems being solved:**

1. **The incumbents have replacement drivers.** In Courts, APEX is unsupported with a fixed end-of-life. In Tribunals/SSCS, judicial scheduling runs on ListAssist, which RAM Pathfinder replaces in wave 2. (GAPS, the SSCS case-management system, is retained, not decommissioned.) **In Tribunals/ET — the wave-1 pilot — the incumbent and its replacement driver are not yet established (gap G8.4); this is an open item for the ET-cohort readiness assessment.**[^d13]
...
**Success:** the **Employment Tribunals** jurisdiction migrated to RAM Pathfinder in wave 1[^d13] (its scheduling incumbent replaced — system not yet identified, gap G8.4); the SSCS jurisdiction migrated in wave 2 with ListAssist replaced for SSCS scheduling (GAPS case management retained); every HMCTS Courts judicial region migrated in waves 3+ with APEX/JI retired; downstream consumers integrating via API; future HMCTS programmes building on RAM Pathfinder's APIs.

### Key characteristics

1. **Greenfield, not strangler.** Neither ListAssist nor APEX supports strangler decomposition (the ET incumbent is unassessed — G8.4). RAM Pathfinder is built end-to-end before any user moves; the legacy system in each jurisdiction runs unchanged for non-migrated users during phased rollout. No dual-write, no event bus, no synchronisation layer.
...
5. **Per-jurisdiction, per-region phased cutover[^d8][^d13].** Wave 1 is the Employment Tribunals jurisdiction. Wave 2 is the SSCS Tribunals jurisdiction. Waves 3+ are Courts regions, one HMCTS judicial region per wave, with all applicable in-region roles together. Migrated users do not use the legacy system; non-migrated users do not use RAM Pathfinder. No contention or synchronisation.

6. **One decomposition serves every jurisdiction[^d13][^d12].** The same 11 services cover ET and SSCS alike: non-judge member sub-types — in ET, non-legal ("lay") members from the employer-side and employee-side panels (*provisional, G8.5*); in SSCS, Medical, Disability-Qualified and Disability (Other) Members — are additional JOH types handled by `ram-joh`, `ram-booking` and `ram-sitting`. Panel composition for specific cases and hearing-type tracking live in **external case-management systems**[^d12], not in RAM.
...
Each role can complete its legacy workflow on RAM Pathfinder without re-training, and faster or no slower than the **cohort's incumbent system** (`[ET-INCUMBENT-TBD]` for the ET jurisdiction in wave 1 — G8.4; ListAssist for the SSCS jurisdiction in wave 2; APEX for Courts jurisdictions in waves 3+):
...
- **Phase 0 Foundations**[^d1][^d7][^d9][^d11]: Reference Data sourced from JOH eLinks API + MRD[^d3] (no ETL, no legacy migration); Authorisation with SSO and the two-population identity model[^d9]; Notification; API contracts (versioned + paper contracts for Itinerary / MI Feed); deployment platform (CI/CD); structured logging conventions[^d7]; stub Home / navigation shell. *(Per-service configuration via Spring profiles + Key Vault; shared `ram_configuration_values` infrastructure table managed by `ram-architecture` Liquibase baseline changelog.)*
...
**Persona:** an ET scheduling/listing admin (exact role title TBC — G8.5). They record absences and ensure Employment Judges and non-legal ("lay") members are scheduled to cover Employment Tribunal sittings. (RAM tells them who is available; the external case-management system tells them what cover is needed — RAM does not manage hearings[^d12].)
...
2. They open the absence. RAM displays the JOH's profile (sourced from `jo_people` per FR11), appointment type (sourced from `jo_appointments`), Specialisations (sourced from MRD), tickets/authorisations (tier-(a) `jo_tickets` + RAM-overlay per FR15), and the dates of absence.
3. They approve the absence. RAM auto-creates a vacancy (FR23) pre-populated with JOH type, jurisdiction (ET), date range.
4. They advertise the vacancy out-of-system (**the ET process is TBC — G8.5**; RAM's availability view per FR18 + FR27 helps them shortlist).
5. Another eligible JOH confirms availability.
6. They create a booking in RAM (FR29) — JOH, jurisdiction = ET, tribunal venue, date range, session type. The vacancy is marked filled (FR30).
7. RAM emails the JOH a booking acknowledgement (FR32).
8. After the hearings happen (managed externally[^d12]), they confirm the sitting (FR37) in RAM, recording whether the JOH actually sat.
...
**Outcome:** the cover cycle completes in fewer clicks than the ET incumbent (comparison blocked on G8.4). JOH availability and bookings are recorded in RAM; the ET case-management system stays informed via APIs.
...
**Outcome:** Asha completes the cover cycle in fewer clicks than ListAssist. JOH availability and bookings are recorded in RAM; the SSCS case-management system (GAPS, retained) stays informed via APIs; JFEPS payment is unchanged from APEX.
...
| **External case-management systems** (ET case management — system TBC per G8.5; SSCS: GAPS; Courts Listing systems)[^d12] | Outbound (APIs) | From Phase 9 (wave 1 onward) | REST APIs from RAM | RAM exposes JOH availability + booking data for external case-management consumption. No reverse-write into RAM (RAM is the system of record for JOH scheduling). |
| **Future programmes** (Actuals, Scheduling & Listing reforms) | Outbound (APIs) | Post-MVP | REST APIs from RAM | Vision-level; specific contract design happens when programme demand crystallises. (Tribunals removed from "future programmes" — ET is wave 1 and SSCS wave 2[^d13].) |
...
| Service | Representative endpoints |
|---|---|
| JOH (`ram-joh`) | `GET /johs`, `GET /johs/{johId}` *(person record read-only, composed from `jo_people` via the RAM JOH id; `?personnelNumber=` filter available)*, `POST /johs/{johId}/working-patterns`, `POST /johs/{johId}/tickets` *(RAM-owned overlays)* |
...
- **FR8** *(revised v2.2, 2026-05-07)*: Cross-service runtime policy values (e.g. session timeout warnings, batch schedules, feature flags) are stored in a shared `ram_configuration_values` infrastructure table, schema-managed by `ram-architecture`'s Liquibase baseline changelog and SELECT-granted to every RAM Pathfinder service DB role. Updates are made via Liquibase changesets or direct admin SQL — no API service. Per-service configuration scoped to a single service uses Spring profiles + `application.yml` + Azure Key Vault.
...
- **NFR32 — Retention**[^d11][^d3]: Data retention per HMCTS retention schedules. **No data is migrated** — historical transactional data remains in the cohort's incumbent system (`[ET-INCUMBENT-TBD]` for ET — G8.4; ListAssist for SSCS; APEX for Courts); RAM Pathfinder retains only data created in RAM Pathfinder from cutover onward.
...
| **GAPS** | The **SSCS case-management system** (legacy), **retained** — RAM Pathfinder does **not** replace GAPS. In **wave 2**[^d13] RAM replaces the SSCS judicial-scheduling tool (**ListAssist**), not GAPS. GAPS is the SSCS analogue of the external case-management systems that consume RAM's APIs (per D12): it stays in place and consumes RAM's APIs[^d11][^d12]. |
...
| **JI** | Judicial Itineraries — the legacy Oracle APEX system serving the **Courts** jurisdiction. Replaced by RAM Pathfinder in waves 3+[^d13]. |
...
| **ListAssist** | The SSCS judicial-scheduling tool used in the SSCS Tribunals jurisdiction today; the SSCS scheduling incumbent and the **wave-2** UAT parity reference. Replaced by RAM Pathfinder for SSCS in wave 2[^d13]. (GAPS, the SSCS case-management system, is retained.) |
| **Employment Tribunals (ET)** | The HMCTS Tribunals jurisdiction handling employment disputes (unfair dismissal, discrimination, wages and related claims). **The wave-1 pilot jurisdiction for RAM Pathfinder**[^d13]. Its judicial-scheduling incumbent, case-management system, role taxonomy and panel structure are **not yet established in this programme's analysis** — see gaps G8.4 and G8.5. |
...
| **RAM Pathfinder** | The API-driven greenfield platform for HMCTS judicial scheduling this PRD describes. Pilots on the **Employment Tribunals** jurisdiction in wave 1[^d13], replaces ListAssist for SSCS in wave 2, and JI/APEX for Courts in waves 3+. |
...
| **SSCS** | Social Security and Child Support — the HMCTS Tribunals jurisdiction handling appeals on PIP, ESA, Universal Credit, DLA and related benefits decisions. The **wave-2** jurisdiction for RAM Pathfinder[^d13] (was wave 1 under D11). |
| **SSO** | Single Sign-On |
...
| **Tribunal Panel** | A composed group of JOHs sitting together for a single tribunal hearing — typically one Tribunal Judge plus one or more Tribunal Members (in ET: an Employment Judge plus non-legal members). SSCS panel composition is hearing-type-dependent (e.g. PIP appeals typically require Judge + Medical + Disability-Qualified); **ET panel composition rules are unconfirmed — G8.5**. Panel composition itself is out of RAM scope per D12. |
