...
- Git: branch **`story/{story-id}`** for story work (created at dispatch, one branch per story through to the PR; `bugfix/`, `chore/`, `feature/` for anything else); Conventional Commits (`feat:`/`fix:`/`docs:`/`refactor:`/`test:`/`chore:`), imperative, ≤72-char subject; PRs → `main` (trunk-based). **An agent may commit and push its feature branch; it may never write to `main`, force-push, tag, or merge the PR** — that is the human gate.
