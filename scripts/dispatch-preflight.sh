...
#   scripts/dispatch-preflight.sh 1.4
#   scripts/dispatch-preflight.sh 1.4 https://github.com/hmcts/ctam-reference-data.git
#
# WHY THIS EXISTS
...
if [ -z "$story" ]; then
  echo "usage: $(basename "$0") <story-id> [remote-url-or-path]   e.g. $(basename "$0") 1.4" >&2
  exit 2
fi
[ -f "$STATUS_FILE" ] || { echo "error: no sprint-status.yaml at $STATUS_FILE — run bmad-sprint-planning" >&2; exit 1; }

dashed="${story//./-}"                       # 1.4 -> 1-4
epic_num="${story%.*}"                       # 1.4 -> 1
epic="epic-${epic_num}"
problems=0
...
# epic-to-epic only, so nothing stopped 1.4 being dispatched while 1.1 had not been built. The
...
if printf '%s' "$story" | grep -qE '^[0-9]+\.[0-9]+$'; then
  story_leaf="${story##*.}"
  ...
  while IFS= read -r sline; do
    ...
    leaf=$(printf '%s' "$skey" | awk -F- '{print $2}')
  ...
  warn "story id '$story' is not <epic>.<story> — skipping the intra-epic ordering check"
fi
