...
The original ctam-analysis session had ~6 MB of binary docs to analyse. Loading them into context wasted tokens and produced lower-quality reads than working from already-extracted plain text. The shared `.claude/lib/_shared/scripts/distil-binary-data.sh` writes UTF-8 `.txt` files into `<input-folder>/output/extracted-text/` and the analysis works from those.
